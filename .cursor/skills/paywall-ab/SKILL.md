...
```
Run paywall-ab skill in evaluate mode.
app_id: anicca
...
```bash
# Conversions: rc_trial_started_event の presented_offering_id でセグメント
# ⚠️ 正しいプロパティ名は presented_offering_id（offering_id ではない）
mcp__mixpanel__run_segmentation_query:
  project_id: {app.mixpanel_project_id}
  event: "rc_trial_started_event"
  from_date: "{experiment_start_date}"
  to_date: "{today}"
  segment_by: "properties['presented_offering_id']"
→ Variant A: {variant_a_offering_id} の件数 = a_conversions
→ Variant B: {variant_b_offering_id} の件数 = b_conversions
```

**Impressions（パイウォール表示数）:**
```bash
# paywall_viewed の offering_id でセグメント
# ⚠️ iOSコード側で offering_id プロパティを送信していること（AnalyticsManager.trackPaywallViewed に追加済み）
mcp__mixpanel__run_segmentation_query:
  project_id: {app.mixpanel_project_id}
...
→ Variant A: {variant_a_offering_id} の件数 = a_impressions
→ Variant B: {variant_b_offering_id} の件数 = b_impressions
```

**⚠️ RC v2 GET /experiments/{id} は analytics を返さない（404）。MixpanelのみがSSOT。**
