...
# ADDENDA A23-A33 — Bible 完全準拠パッチ (2026-04-16, ダイス指示)

**目的:** Welcome / Gender / Struggles / PaywallPrimer / PlanSelection / ValueTimeline / NudgeTimes / Comparison / Retention Offer の Bible 完全整合。すべてのテキスト変更に**1文字単位の Bible 引用**を付ける。すべての UI 変更に **EN+JA ASCII 全画面**を再掲。

## A23. Welcome 完全書き換え (Sign in with Apple 削除 + Bible-aligned headline)

### A23.1 Sign in with Apple 完全削除

**根拠:**
- B1 Screen 1 L141: `"Log in" text link (only if user wants sign-in)`
- ダイス決定 (2026-04-16): アカウント機能不在 → sign-in を持たない → "(only if)" 条件を満たさない → 削除

**ファイル:** `aniccaios/aniccaios/Onboarding/WelcomeStepView.swift`

| 行 | 削除 |
|---|---|
| L2 | `import AuthenticationServices` |
| L7 | `@State private var isRestoring = false` |
| L52-93 | SignInWithAppleButton ブロック全削除 |
| L97-106 | `.disabled(isRestoring)` + `.overlay { ProgressView }` |

### A23.2 Welcome 見出し再導出

**Bible 引用 (1文字単位):**
- B1 Screen 1 L138: `Bold headline stating the transformation outcome (not the app name)`
- B1 Screen 1 L144 Pattern: `"Welcome to your new [transformation outcome]" + device preview showing the app's best screen.`

**Anicca の transformation outcome 確定:**
- Before: 「不安のループ」「考えすぎ」
- After: 「静かな心」「quiet mind」
- 該当語 = `quiet mind` / `静かな心`

**新コピー (Bible verbatim パターン適用):**

| Key | EN | JA | 引用元 |
|---|---|---|---|
| `onboarding_welcome_title` | `Welcome to your new quiet mind.` | `新しい、静かな心へ。` | B1 L144 `Welcome to your new [transformation outcome]` を verbatim 適用、`[transformation outcome]` = `quiet mind` / `静かな心` |
| `onboarding_welcome_subtitle` | `Anicca breaks the anxious loop in 60 seconds.` | `Anicca が60秒で不安のループを断ち切ります。` | B1 L138 `transformation outcome` を補強。アプリ名は subtitle に降格（headline では Bible L138 「not the app name」遵守） |
| `onboarding_welcome_cta` | `Get Started` | `はじめる` | B1 L140 `"Get Started" primary CTA` verbatim |

**ダイス提案「アニッチャへようこそ」却下理由:**
B1 Screen 1 L138 が明示的に `(not the app name)` と禁じている。Bible 違反 = オリジナル = 罪 (CLAUDE.md 根本原則)。ダイス自身のルール「バイブルに従って書かないとオリジナルじゃダメだよ」に従い、Bible verbatim パターンを優先。

### A23.3 Welcome ASCII 完全再掲 (EN)

```
┌────────────────────────────┐
│ ●─────○─○─○─○─○─○─○─○─○─○ │  ← progress bar (B1 L142)
│                            │
│                            │
│                            │
│                            │
│  Welcome to your new       │  ← onboarding_welcome_title
│  quiet mind.               │     (B1 L144 verbatim pattern)
│                            │
│  Anicca breaks the         │  ← onboarding_welcome_subtitle
│  anxious loop in 60 sec.   │
│                            │
│       ┌──────────┐         │  ← device preview (B1 L139)
│       │          │         │     real Anicca screenshot
│       │  Anicca  │         │
│       │  daily   │         │
│       │  view    │         │
│       └──────────┘         │
...
│                            │
│  ┌──────────────────────┐  │
│  │     Get Started      │  │  ← onboarding_welcome_cta (B1 L140)
│  └──────────────────────┘  │
│                            │
│  (no log-in link)          │  ← B1 L141 "(only if)" not satisfied
│                            │
└────────────────────────────┘
```

### A23.4 Welcome ASCII 完全再掲 (JA)
...
│                            │
│                            │
│  新しい、静かな心へ。      │  ← onboarding_welcome_title
│                            │
│  Anicca が60秒で           │  ← onboarding_welcome_subtitle
│  不安のループを断ち切ります。│
│                            │
│       ┌──────────┐         │  ← デバイスプレビュー
│       │          │         │
...
## A24. Name 質問 → Gender 質問 (plain text、icon無し)

**Bible 引用:**
- B1 Screen 4 L168: `Each testimonial has: name/initials, persona tag (e.g. "Busy professional", "Beginner"), review text`
- B1 Screen 4 L169: `Tags should match the audience segments from Screen 2`
- B5 (aso-skills/onboarding-optimization) L?: `Never ask for data you won't use`

**使用コミットメント:** Gender は S6 SocialProof の testimonial 並び替え (man → male persona testimonial を最上位、woman → female persona testimonial を最上位) に**実際に使用する**。これにより B5 「データ収集 = 使用前提」を満たす。

**プレーンテキスト確定根拠 (ダイス指示 2026-04-16):**
> "even for the visual for the gender, I don't think it has to be an icon and shit like that. They can be just male, female, other"

Bible 側に icon 必須の規定なし (B1 Screen 2 L150 の emoji icon は **goal question** に限定)。Gender は goal question ではないため emoji 不要。

**コピー:**

| Key | EN | JA | 引用元 |
|---|---|---|---|
| `onboarding_gender_title` | `Which best describes you?` | `あなたに当てはまるのは？` | B1 Screen 2 形式の self-identification 質問 |
| `onboarding_gender_subtitle` | `We use this to match you with people on similar journeys.` | `似た歩みの人と結びつけるために使います。` | B5「使う前提でのみ訊く」明示 |
| `onboarding_gender_male` | `Male` | `男性` | ダイス指示 verbatim |
| `onboarding_gender_female` | `Female` | `女性` | ダイス指示 verbatim |
| `onboarding_gender_other` | `Other` | `その他` | ダイス指示 verbatim |
...
## A25. Struggles 13選択肢 → 6選択肢 (Bible 5-7 上限遵守)

**Bible 引用:**
- B1 Screen 3 L158: `Multi-select list of 5-7 pain points`
- B1 Screen 3 L162: `"Lack of time" works. "Suboptimal workflow" does not. Use the language real users would use.`

**現状違反:** `StrugglesStepView.swift` L7-14 が13個 → Bible 上限 7 を超過。

**6個に統合 (mapping):**

| 新 key | EN | JA | 統合元 (旧13個から) | Bible 引用 |
|---|---|---|---|---|
| `struggle_overthinking` | `My head won't stop racing.` | `頭の中が止まらない。` | rumination + obsessive | L162 `language real users would use` (一人称・口語) |
| `struggle_avoidance` | `I keep putting things off.` | `先延ばしばかりしている。` | procrastination | L162 一人称 |
| `struggle_anxiety_loop` | `Anxiety hijacks my day.` | `不安に1日を奪われる。` | anxiety + cant_wake_up + staying_up_late | L162 emotional resonance |
| `struggle_self_attack` | `I'm hard on myself.` | `自分を責めてしまう。` | self_loathing + anger | L162 一人称 |
| `struggle_compulsion` | `I can't stop a habit I hate.` | `やめたい癖をやめられない。` | porn_addiction + alcohol_dependency + lying + bad_mouthing | L162 generic で恥ずかしさ回避 |
| `struggle_disconnect` | `I feel alone in this.` | `この状態を一人で抱えている。` | loneliness | L162 emotional |
...
│  Pick all that apply.      │
│                            │
│  ☐ My head won't stop racing.   │
│                            │
│  ☐ I keep putting things off.   │
│                            │
│  ☐ Anxiety hijacks my day.      │
│                            │
│  ☐ I'm hard on myself.          │
│                            │
│  ☐ I can't stop a habit I hate. │
│                            │
...
## A26. PaywallPrimer に transformation restatement + featured testimonial 追加

**Bible 引用:**
- B1 Screen 14 L292: `App logo + headline restating the transformation: "Your [goal] sorted with [App Name]"`
- B1 Screen 14 L293: `One featured testimonial/review with star rating`

**現状違反:** PaywallPrimer に transformation restatement なし、testimonial なし。

**追加コピー:**

| Key | EN | JA | 引用元 |
|---|---|---|---|
| `paywall_primer_transformation` | `Your quiet mind, sorted with Anicca.` | `静かな心を、Anicca で。` | B1 L292 `"Your [goal] sorted with [App Name]"` verbatim |
| `paywall_primer_testimonial_text` | `"My anxious loop used to last hours. Now it's over before I notice."` | `「不安のループは何時間も続いていた。今は気づく前に終わる。」` | B1 L293 featured testimonial |
| `paywall_primer_testimonial_author` | `— Mika, 29` | `— ミカ, 29` | B1 L168 name/initials + persona |
| `paywall_primer_testimonial_rating` | `★★★★★` | `★★★★★` | B1 L293 `with star rating` |

---

## A27. PlanSelection に Restore Purchases リンク追加

**Bible 引用:**
- B1 Screen 14 L296: `"More options" or "Restore purchases" secondary link`

**現状違反:** PlanSelectionStepView に Restore Purchases リンクなし。

**追加:**

| Key | EN | JA |
|---|---|---|
| `paywall_restore_link` | `Restore Purchases` | `購入を復元` |

配置: PlanSelectionStepView 最下部、CTA ボタンの下に secondary text link。タップで `RCPurchasesDelegate.restorePurchases()` 呼び出し。

**重要:** `Start your FREE trial` CTA は **削除維持** (ハードペイウォール、トライアル無し)。Bible L295 の `"Start your FREE [trial period]"` は Anicca の hard paywall 方針 (R1) でオーバーライド。

---

## A28. ValueTimeline に 83% 統計バッジ追加

**Bible 引用:**
- B1 Screen 6 L180: `stats should be specific and credible`
- spec R14 確定: `83% report spirals shorten within 2 weeks` は hardcoded 統計

**現状違反:** PaywallValueTimelineStepView L811-814 が "Week 1...Week 12" のみで統計バッジなし。

**追加:**

| Key | EN | JA |
|---|---|---|
| `paywall_value_stat_badge` | `83% report spirals shorten within 2 weeks` | `2週間以内に83%の人が渦が短くなると報告` |

配置: PaywallValueTimelineStepView headline 直下、Week 1 timeline の上。

---

## A29. PreferredNudgeTimes 事前選択削除

**Bible 引用:**
- B5 onboarding-optimization: `Never pre-select options for the user — it inflates fake commitment`
- B1 Screen 8 L221: `preferences that will visibly affect the demo`

**現状違反:** `OnboardingBibleViews.swift` L451 が `@State private var selected: Set<String> = ["morning", "evening"]` で2個事前選択。

**ファイル:** `aniccaios/aniccaios/Onboarding/OnboardingBibleViews.swift`
...
「Continue」ボタンは `selected.isEmpty` の間は disabled。

---

## A30. ComparisonTable に green ✓ / red ✗ アイコン追加

**Bible 引用:**
- B1 Screen 7 L197: `3-4 rows comparing outcomes (green ✓ vs red ✗)`

**現状違反:** `OnboardingBibleViews.swift` L610-687 ComparisonTableStepView がプレーンテキストのみ、icon 無し。

**追加:** 各行に `Image(systemName: "checkmark.circle.fill").foregroundColor(.green)` と `Image(systemName: "xmark.circle.fill").foregroundColor(.red)` を追加。
...
## A31. spec L1090 Gender ban 行の反転
...
**置換:**
```
| Gender 質問 | A24 で復活: SocialProof testimonial の persona matching に使用するため B5 「使用前提」要件を満たす |
```

---

## A32. Adam Lyttle Retention Offer (in-app custom sheet)

**ソース:**
- 一次ソース: ダイス引用 (2026-04-16):
  > "Retention Offer / This is free money most people leave on the table. When a user tries to cancel their subscription, present a 'retention offer'. Set yours at $44.99/year. / Retention Offer Flow / Do NOT show a retention offer any other way, as Apple will reject it."
- Firecrawl 検証結果: Google CAPTCHA / DDG 0件 / GitHub `adamlyttleapps` org 配下 retention 専用 repo なし / x.com Firecrawl 非対応 → **公開Web上での原典確認不可**。ダイス引用文を一次ソースとして採用。

**実装方針 (iOS 18 Apple 自動 Win-Back ではない、custom in-app sheet):**

| 項目 | 値 |
|---|---|
| トリガー | ユーザーが App 内 Settings → Manage Subscription → Cancel を選択したタイミングで、Apple Pay 確認シートが2回目に表示される瞬間 (ユーザー定義 X ボタン押下時) |
| 形式 | カスタム SwiftUI `.fullScreenCover`、native Apple ダイアログとは別 |
| 表示条件 | **キャンセル意図検知時のみ。それ以外では絶対に表示しない (Apple リジェクト要因)** |
| 価格 | $44.99/yr (yearly.b $49.99 から 10% off。ASC で win-back price point として既登録: USA ID `eyJzIjoiNjc2MjA0OTY5NiIsInQiOiJVU0EiLCJwIjoiMTAzMDIifQ`) |
| 購入処理 | StoreKit2 で yearly.b の win-back offer signature を取得し purchase |

**コピー:**

| Key | EN | JA |
|---|---|---|
| `retention_offer_title` | `Exclusive Offer` | `特別オファー` |
| `retention_offer_subtitle` | `Don't miss out on your quiet mind. 10% off, just for you.` | `静かな心を手放さないで。あなただけに10% off。` |
| `retention_offer_price` | `$44.99 / year` | `$44.99 / 年` |
| `retention_offer_cta_accept` | `Cancel anytime · Accept Offer` | `いつでも解約可 · オファーを受ける` |
| `retention_offer_cta_decline` | `No thanks, cancel my subscription` | `不要、解約する` |

**ファイル (新規作成):**
- `aniccaios/aniccaios/Paywall/RetentionOfferSheet.swift` — SwiftUI view
- `aniccaios/aniccaios/Paywall/RetentionOfferTrigger.swift` — cancel intent detection + StoreKit2 win-back purchase
...
**Apple リジェクト回避ルール (絶対):**
1. `RetentionOfferSheet` は cancel intent 以外で**絶対に表示しない**
2. paywall flow 内で表示禁止
3. onboarding 内で表示禁止
4. Settings tap で表示禁止
5. **唯一の表示パス**: ユーザーが Manage Subscription → Cancel を進めた直後

---

## A33. spec L1090 + A18 Win-Back 行の修正

**現状 A18 が iOS 18 自動 Win-Back を前提にしている部分を修正:**

`A18. Win-Back Offer` セクション全体を以下に置換:

```
## A18. Retention Offer (Adam Lyttle pattern, NOT iOS 18 auto Win-Back)

仕様: A32 参照。
- iOS 18 native Win-Back Offer (ASC `winBackOffers` API) は **採用しない**
- 理由: Adam Lyttle pattern は cancel intent 検知 → custom in-app sheet。
  iOS 18 native は Settings → Subscriptions の OS 側で表示され、Anicca の paywall ロジックを通らない
- ASC win-back price point ($44.99) は yearly.b の StoreKit purchase 時に
  promotional offer signature 経由で適用 (Server-side signing が必要、
  apps/api 側で実装)
```
...
**確認結果 (asc CLI 直接検証 2026-04-16):**

| Variant | Subscription | Intro Offer 件数 |
|---|---|---|
| A annual `6755320744` | ai.anicca.app.ios.annual | **0** (no trial) |
| A monthly `6755320627` | ai.anicca.app.ios.monthly | **0** (no trial) |
| B yearly `6762049696` | ai.anicca.app.ios.yearly.b | **0** (no trial) |
| B weekly `6762049888` | ai.anicca.app.ios.weekly.b | **0** (no trial) |

**結論:** 4商品すべて trial なし。ダイス確認情報と一致。Hard paywall (R1) と矛盾なし。

**判断: A/B 両方継続。**

理由:
1. 両 variant が hard paywall 方針と矛盾しない (trial 全廃済み)
2. RC Experiment `prexpc5c110e6f6` は status=running、total_running_time=327967秒 (≈ 3.8日)。14日達成まで継続が統計的に意味がある
3. B のみに絞ると A/B test の目的が消滅し、勝者選定不能
4. 1.8.5 リリース時: 両 variant に対して新 onboarding (A23-A33) を提示し、paywall 部分のみ A/B 維持
