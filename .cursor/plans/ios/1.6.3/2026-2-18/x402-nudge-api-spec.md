...
| 12 | buddhist-counsel SKILL.md 作成 | ⬜ 未 | skillcraft |
...
| # | タスク | 状態 | 使うツール |
|---|--------|------|-----------|
| 14 | x402-skill-marketer SKILL.md 作成 | ⬜ 未 | skillcraft（商品が増えるたびに cron を書き直すのは地獄。スキルにすれば「商品リスト」を渡すだけ） |
| 15 | VPS にスキルインストール | ⬜ 未 | scp + openclaw skills install |
| 16 | Cron の prompt を「Execute x402-skill-marketer skill」に書き換え | ⬜ 未 | jobs.json 部分更新 |
| 17 | 手動テスト（Moltbook 実投稿 + #metrics 報告確認） | ⬜ 未 | VPS で実行 |
