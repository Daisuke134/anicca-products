...
│   │   │   ├── promptSanitizer.js
│   │   │   └── __tests__/
│   │   ├── index.js                 ← x402Router をマウント（L34, L70-71）
│   │   └── ...
│   ├── services/
...
- `server.initialize()` を try-catch で囲み、失敗しても続行（syncFacilitatorOnStart: false 相当）
...
│   │   └── promptSanitizer.js        ← apps/api から移動（import パス修正）
│   ├── services/
│   │   └── buddhistCounselService.js  ← apps/api から移動
│   ├── lib/
│   │   └── prisma.js                 ← DB 接続（同じ PostgreSQL、connection_limit=5）
│   └── server.js                     ← 新規（Express + x402 + /health + graceful shutdown）
├── prisma/
...
3. x402 手動初期化: x402ResourceServer + ExactEvmScheme + HTTPFacilitatorClient
   → server.initialize() を try-catch で囲む（失敗しても起動継続）
4. paymentMiddleware で全8ルートの課金設定を一括登録
5. 各ルートをマウント（/emotion-detector, /buddhist-counsel, 等）
6. SIGTERM ハンドラ（graceful shutdown）
   → server.close() → prisma.$disconnect() → process.exit(0)
7. ポート: process.env.PORT || 3001
```
...
（x402/ ディレクトリ + buddhistCounselService.js 削除で完全ゼロ）
...
@coinbase/x402, @x402/evm, @x402/express, @x402/extensions, ethers,
@x402/core (peer dependency), dotenv
```
...
| やること |
|---------|
| `apps/x402-agents/` を新規作成（Express + x402 + Prisma + 8ルート + graceful shutdown） |
...
| `apps/api/src/routes/x402/__tests__/` | 削除 |
...
Step 1:  apps/x402-agents/ を作成
         ...
         - src/server.js（Express + x402 初期化 + paymentMiddleware + 8ルートマウント + /health + graceful shutdown）
...
Step 6:  両サービスのデプロイ確認
         - x402-agents: /health → 200, payment middleware active ログ確認
         - 既存 API: /health → 200, /api/x402/* → 404

Step 7:  iOS の NetworkSessionManager を修正（接続数 + タイムアウト）

Step 8:  fastlane test

Step 9:  main に push → Production デプロイ

Step 10: 動作確認（AC-1 〜 AC-10）
```
