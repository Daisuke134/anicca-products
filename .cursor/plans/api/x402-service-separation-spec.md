...
| **ブランチ** | `dev` |
| **ベースブランチ** | `dev` |
| **作業状態** | Spec v4（レビュー反映: blocking 3件 + advisory 5件修正） |
| **対象** | API + iOS |
...
─────────────────────────────
1. Express app 作成
2. CORS 設定（origin: '*' — x402 は payment が認証のため origin 制限不要）
3. Rate limiting: express-rate-limit 30 req/min（既存 API と同じ設定を移植）
4. /health エンドポイント（ヘルスチェック）
5. x402 手動初期化: x402ResourceServer + ExactEvmScheme + HTTPFacilitatorClient
   → server.initialize() を try-catch で囲む（失敗しても起動継続）
6. paymentMiddleware で全8ルートの課金設定を一括登録
7. 各ルートをマウント（/emotion-detector, /buddhist-counsel, 等）
8. SIGTERM ハンドラ（graceful shutdown）
   → server.close() → prisma.$disconnect() → process.exit(0)
9. ポート: process.env.PORT || 3001
```
