...
| Mac-local runtime | Release `8a9fffcd17055a766d16a3a6ebcb90f18736a91f` is installed and the authenticated local foundation is GREEN | Installer restart restored the dedicated EN tab at `/app/home`, then the installed wake returned `AUTHENTICATED + READY_FOR_PUBLICATION`; unchanged wakes preserve transition ID `c1702e21134601f7ae060c3f2c3c4c69737d6e6d539d869bb117c5521d9e020f` with `provider_changed=false`. `ai.anicca.affiliate-browser` is running, `ai.anicca.affiliate-loop` exits `0` on a 600-second interval, browser/provider startup races retry within bounded windows, and repeated `article-1` resolution keeps exactly one placement row. This proves readiness, not publication or revenue |
