...
| X publication | No Affiliate X placement exists yet. The canonical skill has a dedicated `x post publish` boundary that verifies `@selawmqt:9326`, requires disclosure plus one owned article URL, independently requires that URL's owned-publication receipt to be `LIVE`, writes an effect-possible fence before the click, reconciles exact timeline text after an ambiguous retry, and requires post-page readback before `LIVE`. X's April 2026 rules warn that scripted website automation may permanently suspend an account | The source-bound X artifact is `READY_FOR_X_PUBLICATION` with SHA-256 `ba64a7eaa30e464cdfb6e4e12faadbe6c8973cef88a034a269b3bba7e1080fa8`; no Publish click has occurred yet. The user-selected implementation is browser-only. Keep action caps and immediate account quarantine; never describe this lane as platform-approved or evade challenges |
