...
| F0 current-Mac bootstrap | Core primitives GREEN and installed disabled at `615206fd98fb555b0aada794454dd63e1cc95260`; current-host capability admission is the active slice | Bootstrap 3/3, authority 2/2, profile isolation 2/2, and R0 regression 3/3 pass. The next receipt inventories explicitly requested local executables/app bundles by canonical path, version, size, and SHA-256 without launching or connecting to them. Measured candidates are CloakBrowser Chromium `145.0.7632.109`, Chrome `151.0.7922.138`, Python `3.14.6`, and macOS system tools. The default manifest still contains no invented download; Keychain readback, authorized-session discovery, and account-handle proof remain open |
