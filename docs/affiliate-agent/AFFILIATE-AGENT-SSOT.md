...
| ElevenLabs | The official affiliate entry reached ElevenLabs signup. The authorized email passed Arkose, the existing account was recovered through the official email reset, and a storage-isolated fresh login loaded `/app/home`. The login read its password from the Git-external mode-0600 private Markdown, not browser state or Keychain. The authenticated home now renders the PartnerStack-managed Affiliate Program signup CTA; no affiliate activation or link is claimed yet | `AUTHENTICATED_APPLICATION_READY`; keep the private Markdown as credential SSOT, Keychain only as a mirror, submit the real PartnerStack application, then require authenticated application/link readback |
...
| Provider auth | HubSpot/Impact is `APPLICATION_PENDING + AUTH_RECOVERY_PROVIDER_DEFECT`; ElevenLabs is `AUTHENTICATED_APPLICATION_READY`; Kit is `APPLICATION_REJECTED`; Systeme.io is `EXTERNAL_CHALLENGE` at visible reCAPTCHA; Amazon JP is `AUTH_RECOVERY_OTP_REQUIRED`; Rakuten remains `AUTH_REQUIRED` | ElevenLabs credential recovery and isolated fresh login are proven from the private Markdown SSOT. Its affiliate application is not yet submitted, and no provider approval, tracking-link ownership, current executable offer, or payout setup is proven |
