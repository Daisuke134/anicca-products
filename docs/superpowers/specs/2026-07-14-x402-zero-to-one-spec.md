...
| 1b | Fable(今) | demand 面の追加: x402scan 掲載確認・Agent402 index・directory PR follow | 各面で発見可能を実測 | partial — awesome-x402 PR #838をupstream mainへrebaseし、head `1b0b1e3c`、1 file/1 insertion、MERGEABLE/CLEAN、31/31 endpoint=402。canonical x402 repoはecosystem pageをsunset済みのため既存#2532とdevelopment fork #190をcloseし、無効なreview待ちを解消。Onchainはcontact-onlyのためintegration問い合わせ済み(id `9ecaaca6…`)。Ampersendはself-submit非公開のため公式連絡先へcuration依頼済み(id `1470ef06…`)。QuestflowはQDP/A2A障害中のため公式resource PR #11提出(head `ea40c3f0`, Verified, CLEAN)。Agent402は31 tools、health=1、routableで掲載され、buyer route検索にも出る。metadata誤変換はupstream PR #473で修正提出済み(head `b8504531`, MERGEABLE)。x402scanはAgentCash SIWX署名付き登録で31 endpointの発見までは成功したが `422 no_valid_resources`、31/31がv1として拒否。`serve-v2.mjs`無変更のDIST範囲では掲載不能。残りは有効PR review/merge、Onchain/Ampersend返答、別タスクでのv2移行 |
