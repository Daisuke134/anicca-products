...
- Agent402 route metadataだけは `/research` を `POST`, `price:null/$0` と返すが、公開OpenAPIと実402は `GET`, `$0.003`。掲載PASSを覆さないためDISTは進め、buyer実行精度の上流metadata課題として残す。
...
| 1b | Fable(今) | demand 面の追加: x402scan 掲載確認・Agent402 index・directory PR follow | 各面で発見可能を実測 | partial — awesome-x402 PR #838をupstream mainへrebaseし、head `1b0b1e3c`、1 file/1 insertion、MERGEABLE/CLEAN、31/31 endpoint=402。Coinbase PR #190修復(head `9d18c7b6`, Verified, mergeable)。Onchainはcontact-onlyのためintegration問い合わせ済み(id `9ecaaca6…`)。QuestflowはQDP/A2A障害中のため公式resource PR #11提出(head `ea40c3f0`, Verified, CLEAN)。Agent402は31 tools、health=1、routableで掲載され、buyer route検索にも出る。x402scanはAgentCash SIWX署名付き登録で31 endpointの発見までは成功したが `422 no_valid_resources`、31/31がv1として拒否。`serve-v2.mjs`無変更のDIST範囲では掲載不能。残りは各PR review/merge、Onchain返答、別タスクでのv2移行 |
