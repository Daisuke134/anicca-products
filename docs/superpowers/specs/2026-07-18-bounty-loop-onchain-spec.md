...
- B4 acquisition experiment A1 checkpoint: finalized head `48983771`（窓の955/43,200 blocks、残り42,245 blocks）時点で、A1開始後の3店telemetryは合計720 attempts（うち`/image`は43）だが`settled=true=0, payer=0, tx=0`。3 walletのfresh 48h recorderも全て`settledTelemetry=0, verified=0, recorded=0`で、売上ではなくcrawler probeだけを観測する。窓は未終了なのでA1の成否は未判定、価格・metadata・listingは凍結を維持する。
- B4 acquisition A2 read-only qualification: [the402 provider guide](https://the402.ai/docs/providers#registration)はGoogle/emailを使わないprogrammatic x402 self-registration（外部platformへの$0.01費用）でAPI keyとwebhook secretを返し、signed webhookによる自動fulfillmentとrequest bidを提供する。公開catalogの`image generation`は4件だが、service detailを再計測すると$0.05 Image Generation自体は`total_jobs=2, successful_jobs=0, failed_jobs=2`であり、provider全体の成功112件をimage需要と誤読しない。一方、公開postingにはwriting/research各1件、budget `$1–10`、bid 10/9件が実在し、human credential不要で作れるHTTP 402 explainerとx402 adoption briefを要求する。A1終了までは登録・listing・bidを行わず、A1が0ならHMAC/replay/idempotencyをTDDした専用webhookとresearch service/bidをA2の単一要因として使う。ソース: [the402 Provider Onboarding Guide](https://the402.ai/docs/providers#webhook-reference) / 核心の引用: 「For programmatic onboarding, call the registration endpoint directly.」「All webhook payloads are signed with HMAC-SHA256 using your webhook_secret」 / [the402 open postings](https://api.the402.ai/v1/postings?status=open&limit=100) / 核心の引用: 「Research brief: x402 payment adoption among AI agent frameworks」
- B4 acquisition A2 dormant safety prep: unpaid `POST /v1/register`のlive 402 challengeを再計測し、`x402Version=1`、`exact`、Base、10,000 atomic USDC、resource、外部recipientを完全一致で検証するfail-closed guardが通る。専用receiverはAPI-key header、`${timestamp}.${rawBody}`のHMAC-SHA256、±300秒replay窓、event type/idを検証した後、SQLite PRIMARY KEYでACK前にdurable enqueueする。再起動dedupe、lease token、期限切れreclaim、stale worker拒否、bounded retry/DLQ、0600 DB、1 MiB body上限を持ち、audit/HTTP responseはbuyer payload・raw body・header・secretを返さない。新規12 testを含む全197/197 testがPASS（anicca `9085f2fa`,`4dc5076d`）。fulfillment payloadは処理用DBだけに保存してlogへ出さない。ただしworker/fulfillment/callback、runtime route、登録/payment、listing、bidは未実装・未実行であり、A1終了前には有効化しない。現行Node v25.6.1は`node:sqlite`のExperimentalWarningを出すため、activation前にruntime versionを再検証する。[the402 Provider Guide](https://the402.ai/docs/providers/#bidding-on-requests) / 核心の引用: 「Delivery is at-least-once — consume idempotently keyed on `posting_id`.」 / [SQLite UPSERT](https://www.sqlite.org/lang_upsert.html) / 核心の引用: 「If the insert operation would cause the conflict target uniqueness constraint to fail, then the insert is omitted」 / [Node.js Crypto — `crypto.timingSafeEqual`](https://nodejs.org/api/crypto.html#cryptotimingsafeequala-b) / 核心の引用: 「This function compares the underlying bytes representing the given ArrayBuffer, TypedArray, or DataView instances using a constant-time algorithm.」
...
- x402市場の総需要とbuyer/category/supplyはscoutへ入り、3店のimage listing/402 telemetryとB4 recorderも成立するが、external inflowは$0。A1価格実験でAgent402 default top20入りは成立する一方、第三者purchaseは未観測で、Agentic/Bazaarのimage掲載は第三者による最初のverify+settleがgate。A1終了までは第三者paid purchaseを観測し、0なら専用signed webhookを持つthe402 research request bidを次の単一要因として開始する。open posting、bid、award、escrowは収益に数えず、外部USDC releaseとledger記録まで本番証明は未完。
