...
- B4 acquisition A2 read-only qualification: [the402 provider guide](https://the402.ai/docs/providers#registration)はGoogle/emailを使わないprogrammatic x402 self-registration（外部platformへの$0.01費用）でAPI keyとwebhook secretを返し、signed webhookによる自動fulfillmentを提供する。公開catalogの`image generation`は4件で、$0.05のImage Generationが`webhook_healthy=true, provider_completed_jobs=112`を持つため、第三者buyer demandの実績がある。A1終了までは登録・listingを行わず、A1が0ならHMAC/replay/idempotencyをTDDした専用webhookをA2の単一要因として使う。ソース: [the402 Provider Onboarding Guide](https://the402.ai/docs/providers#webhook-reference) / 核心の引用: 「For programmatic onboarding, call the registration endpoint directly.」「All webhook payloads are signed with HMAC-SHA256 using your webhook_secret」
...
- x402市場の総需要とbuyer/category/supplyはscoutへ入り、3店のimage listing/402 telemetryとB4 recorderも成立するが、external inflowは$0。A1価格実験でAgent402 default top20入りは成立する一方、第三者purchaseは未観測で、Agentic/Bazaarのimage掲載は第三者による最初のverify+settleがgate。A1終了までは第三者paid purchaseを観測し、0なら専用signed webhookを持つthe402 A2だけを次の単一要因として開始する。recorderが実txをwallet別ledgerへ記録する本番証明は未完。
