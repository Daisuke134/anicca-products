...
- B4 acquisition experiment A1 checkpoint: finalized head `48983003`（窓の187/43,200 blocks）時点で、A1開始後の3店telemetryは合計506 attemptsだが`settled=true=0, payer=0, tx=0`。3 walletのfresh recorderも全て`settledTelemetry=0, verified=0, recorded=0`で、売上ではなくcrawler probeだけを観測する。窓は未終了なのでA1の成否は未判定、価格・metadata・listingは凍結を維持する。
