...
- B4 acquisition A2 dormant safety prep: unpaid `POST /v1/register`のlive 402 challengeを再計測し、`x402Version=1`、`exact`、Base、10,000 atomic USDC、resource、外部recipientを完全一致で検証するfail-closed guardが通る。専用moduleはAPI-key header、`${timestamp}.${rawBody}`のHMAC-SHA256、±300秒replay窓、event type/idを検証してstable idempotency keyを返し、auditは`ts/type/eventId/status`だけを許可する。新規5 testを含む全190/190 testがPASS（anicca `9085f2fa`）。ただしdurable idempotency store/queue、runtime route、登録/payment、listing、bidは未実装・未実行であり、A1終了前には有効化しない。[Node.js Crypto — `crypto.timingSafeEqual`](https://nodejs.org/api/crypto.html#cryptotimingsafeequala-b) / 核心の引用: 「This function compares the underlying bytes representing the given ArrayBuffer, TypedArray, or DataView instances using a constant-time algorithm.」
