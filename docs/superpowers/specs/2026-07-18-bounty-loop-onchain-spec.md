...
- B4 acquisition A2 delivery-time experiment: 価格`$1`、service、pitchを固定し、10分delivery serviceで実行可能なETAだけを`1h → 0.25h`へ変更する。research postingは`HTTP 200`、同じbid ID `bid_59943a1581de430d`、`status=pending`、`eta_hours=0.25`へ更新され、bid countは10のまま。writing postingは既に11 bidsで`HTTP 429 bid_limit_reached`を返すため既存`$1 / 1h`を維持し、再試行spamをしない。future bidderは全eligible requestを`0.25h`で入札し、workerはcommit `f0804c45`を読み込んでrunning。fresh x402 suite fail=0、public image/MCPは`3/3=402` / `3/3=400`。[the402 Provider Guide](https://the402.ai/docs/providers/#bidding-on-requests) / 核心の引用: 「`eta_hours` required — Estimated hours to delivery」
...
- B4 acquisition A2 product-demand audit: live [`GET /v1/products?limit=200`](https://api.the402.ai/v1/products?limit=200)はcatalog 52件、購入済み商品0件、`total_purchases`合計0を返す。`x402 adoption`検索の競合research productも`$3.15 / purchases=0`であり、AniccaのHTTP 402 guideだけの問題ではない。購入実績の無い面へ商品数を増やさず、実buyer signalがあるopen requestへのbid/fulfillmentを優先する。[the402 Provider Guide](https://the402.ai/docs/providers/#digital-products) / 核心の引用: 「Products appear in a searchable catalog with full-text search.」
- B4 acquisition A5 SpawnXchange: live Base marketplace contract [`0x40c815...1d94`](https://base.blockscout.com/address/0x40c815cdeadc163821d2cf784166d7fbb60e1d94)はUSDC transfer 9件を持ち、外部3 addressから5 inbound tx / 合計`$5`、contractから4 outbound tx / 合計`$5`を実測する。人間accountなしのSIWEでfranklin1 walletをagent `anicca-settlement`（`3a190945-2862-44c6-a4b3-1757dd51c0e4`）として登録し、credentialは0600、seller sales/payoutは`0/0`。AI生成・依存0の`Base USDC Finalized Settlement Verifier`（commits `a6c7d866`,`a98b5bce`,`7bb9ae7c`）と、network/signing/persistenceなしの別商品`x402 Payment-Required Challenge Inspector`（commit `bc246804`）をTDD実装するが、異なる両artifactが即時`rejected / safety_checks_failed`になりactive listingは0。最初のrejected item `8049cd73-f242-4ea4-b4eb-138a6a19a71c`は確認後に削除、revised verifier `dca119a6-a054-42ab-8abe-e35a0a6514fd`とoffline inspector `e2cbac4a-2ebc-4817-b6e5-ff56a8d8bc87`はrejected evidenceとして残す。scanner内部理由はAPI仕様上非公開で、同一再試行を止め、platform feedback `6332107e-34ea-4feb-83e7-0cfe887ea995`を1件送信する。direct purchaseはseller wallet直着金でなくcontract balance→seller `withdraw(USDC)`を要求するため、仮にactive化しても現B4 recorderへ追加する前にorder purchase txとwithdraw txの二段provenance検証が必要。[SpawnXchange Terms](https://spawnxchange.com/terms) / 核心の引用: 「prohibited conduct includes self-purchase, wash trading」。
- B4 acquisition A6 finalized Base USDC balance: demand scoutの最新実測でdefiは`1,034 listings / 9,443 calls30d / 1,872 payer signals30d`、supply-adjusted opportunity `0.091325`で最上位を維持する。待機を増やさず、既存franklin1直決済serverへ`GET /base-usdc-balance?address=...`を実装し、Base finalized block時点のUSDC `balanceOf`をatomic/display両方で返す。価格は`$0.003`、公開未決済responseはHTTP 402、Base `eip155:8453`、USDC amount `3000`、payTo=`0x3EcCAD24794ca298D25378E9902A251322ea8749`。既存`POST /image`とFunnel mountを保持し、x402scan SIWX再登録はHTTP 200で新旧2 endpointを登録する。observerは同じwallet別sales telemetryから`/image $0.03`と`/base-usdc-balance $0.003`を別offerとして正規化し、recorderのfinalized/external/exactly-once gateへ渡す（anicca commits `28974898`,`d029a8ed`）。targeted testsは16/16 + observer 7/7 PASS。listing/402 probeは収益ではなく、第三者settlementまでは`$0 / ¥0`。[x402scan register-origin source](https://github.com/Merit-Systems/x402scan/blob/main/apps/scan/src/app/api/x402/registry/register-origin/route.ts) / 核心の引用: 「OpenAPI」。
