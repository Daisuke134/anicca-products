...
- B3 distributionはfranklin1を完了。`ai.anicca.image-franklin1`がrunning、既存Funnel `443`の`/`・`/mcp`を維持して`/image`を追加し、public `POST /image`が402・50,000 atomic USDC・正しいresourceを返す。franklin2/claude-p配置とdirectory listingは未完（anicca `1fc1e445`,`ccc5cd8c`,`054589f9`、test 167/167 green）。

**残タスク（上から順）:**

| # | 残タスク | done条件 | 依存 |
|---|---|---|---|
| B3 | **distribute + observe（1/3配置済）** | franklin1 public 402 ✅。franklin2/claude-pへimage serverをlaunchd配置し、public `POST /image`をBazaar/MCP directoryへ掲載。listing URLとアクセス/402 telemetryを保存 | B2 ✅ |
...
- **Phase 2 — product + distribution（ACTIVE: B2 COMPLETE / B3 1/3）**: image productの需要・unit margin・167/167 greenを確認。franklin1はlaunchd runningかつlocal/public 402。exit proof残り = franklin2/claude-p public endpoint、listing URL、request telemetry。
