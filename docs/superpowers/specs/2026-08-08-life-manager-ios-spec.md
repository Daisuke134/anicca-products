...
- The single permitted fresh Sol review of the integrated Gate 8 diff returned `fix-first` with four reproducible release blockers. The fixes are now integrated through `7185f4183`: the last successfully registered APNs token is persisted only after a successful registration and survives relaunch for account deletion; outbox commit atomically creates a durable APNs job with lease, bounded provider timeout, retry/backoff, multi-device failure isolation, terminal success, and credential-missing no-op health; a successful question reply forces a fresh durable sync after a concurrent refresh; and the account-deletion receipt retains the confirmed product locale after profile removal. Integrated `bundle exec fastlane test` is 140/140 GREEN (2 UI plus 138 unit), and `node --test test/mobile-*.test.js` is 207/207 GREEN. The full backend suite reaches only the pre-recorded unrelated legacy-path scanner failure for `connector-host-bridge-boot.sh` and `deploy-connector-runtime.sh`; those files are outside this gate and remain unchanged. The real Japanese flow and real APNs receipts remain required; no second review round is added.
