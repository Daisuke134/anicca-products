...
- **REPO 8c.Rはdesign approved / pending**: ID `1273052304`を`life-manager-v0`へ退避してpublic/unarchivedで保持した後、ID `1248111245`をfinal `life-manager`へrenameする。remote、ID別refs/issues/stars、旧`anicca` redirect、old `life-manager` takeover、Pages、live URLを全実証するまでdoneにしない。repo設定mutationは未実施で、`anicca-products`/Railway/§10 product runtime side effectは0。設計・rollback→`2026-07-23-life-manager-repository-rename-design.md`、実行→`../plans/2026-07-23-life-manager-repository-rename.md`。
