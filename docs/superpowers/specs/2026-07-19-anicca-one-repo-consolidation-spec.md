...
- **CORE 8d corrective RED完了 / GREEN rescue実行中**: base=`09dbd5ef5`。first GREENはexpanded=`142/142`でも、harnessの`Date.now()+1`推測がproduction send-boundary欠陥を隠し、coverage=`148/149`・collectors lines=`88.80%`のためmanagerがcommit前に停止。dirty diffを保持してfresh rescueへ再発注する。合格はproduction acceptance後境界、harness推測0、exact closure、privacy value-mask 0、coverage 4/4>=90、全suite GREEN。Phase 2c/provider/L3/final-report/deploy/mergeは禁止。
