...
- **PANEL 8d.1 process repair method 3へ**: exact base/head/upstream=`84e1cebae`、fresh review=`FAIL / blocker 10`。attempt 1は30分無増分でkill。attempt 2は公式`recordGate()`が既存`phaseHistory[5..7].details`をschema違反として拒否（false hypothesis=`runtime validator exit 0ならatomic state APIも読める`）。state-repair method 1はさらに未追跡finding 10件中8件で`category=test_coverage`とverdict dimensionが不整合なことを検出し、変更0で停止（false hypothesis=`review本文の逐語保持は誤ったschema metadataまで保持する必要がある`）。method 3は本文/severity/evidence/routeをbyte不変のままcategoryだけをcanonical mappingへ修正し、stateのexact 3 key `details→reason`と合わせてfixture TDD付きone-shot migrationを実行する。全schema PASS後に公式APIだけでPhase 3 FAIL、10 finding beads、`4→2a`を記録する。これが失敗なら同一atomic 3手法FAILとしてfalse hypothesisを固定し次の独立行へ進む。product code、provider、OAuth、Dais account、deploy、merge、L3は禁止。
