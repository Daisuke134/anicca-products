...
| 8f | CORE-c | context/onboarding/discovery: 初回user、既存user、location未解錠/解錠後、同じclosed Qを二度聞かないことを再検証 | eval 100% + 実TG callback + DB/context provenance。質問禁止領域の発話0件 | **pending — genuine RED push済み、corrective RED→GREENへ**。accepted base=`85a68abaa`、RED/head/upstream=`90933cbb9`。production exports直結の固定12ケースevalは`7/12`、失敗5件=`§9.11 closed inline Q`、`TG callback typed provenance`、`later-series semantic dedup`、`live-location source provenance`、`additive tenant-scoped unique schema`。既存calendar/late/関連は`21/21`,`12/12`,`60/60` PASS。fresh material auditで追加2 blockerを再現: web onboarding resumeがDB/user-scopedでなくglobal localStorage依存のためnew browserの既存user再質問・same browser別user状態混在、calendar connectがclient `sig`を検証せずuidだけでstatus/OAuth/provider mutation。fixtureだけで初回/既存をPASSにしないため、clean RED commitで旧Solを停止。次はこの2件をproduction endpoint/UI behavior REDとして別commit後、既存5件と合わせてminimal GREEN。provider side effect=0 | pending |
...
- **CORE 8fはproduction-path REDをpushしcorrectiveへ**: RED=`90933cbb9`、固定12ケース=`7/12`。plain-text ask、callback provenance無し、series跨ぎ重複、location source無し、provenance migration無しをproduction exportで再現。fresh auditはglobal localStorage onboarding resumeとcalendar-connect `sig`未検証もtenant/onboarding blockerと裁定したため、fixture GREENへ進む前に旧Solを停止。次はこの2件のbehavior REDを追加し、合計7 blockerだけをminimal GREENにする。
