...
| 8f | CORE-c | context/onboarding/discovery: 初回user、既存user、location未解錠/解錠後、同じclosed Qを二度聞かないことを再検証 | eval 100% + 実TG callback + DB/context provenance。質問禁止領域の発話0件 | **pending — GREEN/dev merge/exact-SHA staging PASS、production L3へ**。accepted base=`85a68abaa`、journey RED=`90933cbb9`、corrective endpoint/UI RED=`0fc453527`、GREEN=`771f996c9`。corrective REDはserver resume 1、actual client resume/isolation/save/link 4、calendar signature/replay 2の計7件を意図どおりFAIL。fixed context eval=`12/12 (100%)`、実production `/telegram` handlerのcallback/replay/cross-tenant contract、onboarding user-scoped resume、calendar signed connect、full Life Call test/evalはいずれもfail 0。landing full typecheckの2件は既存scope外baseline（dashboard declaration / income wallet props）のまま。PR #338を通常mergeし、merge/origin-dev=`a4e3ae9c62ffeadcfe328298df13b03658946f1b`。Railway staging deployment=`bcd237d9-590b-49e4-9ada-cdebcfa54beb`はexact SHA SUCCESS、health/panel/protected API/invalid webhook smoke=`200/200/401/401`。provider side effect=0。次はadditive migration→dev-to-main通常promotion→exact-SHA production→Dais本人の実TG callback・DB/context provenance・semantic duplicate 0・forbidden questions 0をL3実測する | pending |
