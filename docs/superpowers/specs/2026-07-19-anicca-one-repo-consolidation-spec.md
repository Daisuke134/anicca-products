...
| 8d.1 | PANEL-0 | permanent personalized dashboard access + connection controls: bookmark可能なstable `/panel`、chat intentとpanel操作を同じuser-scoped commandへ統一。connection card・権限・organ automation・通知・call/委任を本人が接続/切断/ON/OFFできる | RED: fresh token 403、24h固定session、未認証raw 401/403、dead link、non-clickable card、cross-user leakage、hardcoded connection/context、chat/panel state drift。GREEN: 5分単回tokenはlogin bootstrapだけ、token交換後はstable `/panel` + rotating/refreshing HttpOnly session、明示logout/rebind/revoke/storage消去まで通常利用を維持。tenant isolation/CSRF/single-use/auth/action/session-rotation contract 100%。L3: 実TG login→fresh HTTP 200→token消滅→bookmarkした`/panel`をbrowser再起動後も直接再訪→本人固有dashboard。clock-advanced testで24h超のrotation/refreshとlogout/rebind/revokeを実証。harmless toggleをpanel→chat readback→chat intent→panel readbackの双方向で実証し、isolated第2userの表示/action混入0。supported connectorはtest userで実OAuth開始+callback、未提供connectorは正直なunavailable。mobile/desktopで全action clickable | **pending — schema・dev merge・staging PASS / production promotion・authenticated L3未了**。release head=`44d9cdf270a8293e42907c7c9f59331f7002d671`、PR #332は通常merge済み、merge/origin-dev=`835e28a5b668802899b946d56963b4be3366d980`。Supabase migration hashes=`da763c54f19997015fbeda24415d70a2f77c71c8879f51537c39b206da0d8462`,`7a812686cf9dfeadaa356c64ec280799df91536c8f032ac8209d1009edb5838e`を単一transactionで適用しschema reload済み。manager fresh postflightはRLS tables=`4/4`、durable columns=`8/8`、protected functions=`6/6`、ACL failures=`0`、obsolete resolver=`0`、legacy sessions total/revoked/active=`4/4/0`、receipts/preferences=`0/0`。Railway staging deployment=`6fa417c6-3d0d-4ff0-93ca-48dca1788b0e`はexact merge SHAでSUCCESS。manager fresh smokeはhealth=`200 ok=true`、`/panel`=`200 no-store / Forbidden=false`、invalid token=`403`、control-center/timeline=`401 unauthorized`、unauth command POST=`401`かつreceipts/preferences=`0/0→0/0`。persistent evidence=`/Users/anicca/.codex/evidence/panel-0-staging-release.md` SHA-256=`962f43f375625c63c2757420f87219647a3d8d57c49e53c7c2882922be71038d`。production preflightで`life-call`は旧mainのため`/panel=401`,`control-center=404`、先にexact-SHA昇格必須。Calendar auth-configは既存enabled Google OAuth2正本とDais所有ACTIVE接続1件をread-only照合済みだがRailway env未配線。一次資料でmanaged OAuth legacy `POST /connected_accounts`は2026-07-03以降400、現実装は旧endpoint + provider `state`上書きと確認したためproduction発注をmerge前に停止。main/dev/env/provider/TG等のside effectは0。設定変更TG応答はgeneric `Setting updated`のため値そのもののchat readbackとは扱わず、§9.11 copyを無断変更しない。次は発注書`order-panel-composio-link-corrective.md`で`POST /connected_accounts/link` + exact `callback_url` + provider redirect不変をRED→GREENし、manager final check後にstaging→production L3へ戻る |
