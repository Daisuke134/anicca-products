...
- **PANEL 8hはNO-STALLのisolated prepのみ開始**: order=`.claude/sol-orders/order-panel-8h-ux-privacy.md`、branch=`sol/panel-8h-ux-privacy`、base=`origin/main@5a61251e`、heartbeat=`.claude/sol-orders/logs/panel-8h-collab-progress.log` mode `0600`。8g production L3 PASSまで8hのPR/merge/deploy/provider/prod/TG/email/call/L3は禁止し、spec/eval/RED/build/GREEN/review/commit/pushだけを許可する。
