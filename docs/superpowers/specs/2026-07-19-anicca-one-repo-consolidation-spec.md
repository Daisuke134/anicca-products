...
- **PANEL 8d.1 standalone releaseはreview FAIL / delegation blocker 1**: current dev=`5a4ec98e9`からPANEL-only 52 pathsを移植しproduct=`492174ed2`、blocked evidence/HEAD=`f1fba91a0`をpush。focused/full/eval/API/UI=`62/316/33/5/6`全PASS、CORE path混入0。ただしexactly-one fresh reviewとmanager独立実測は、`turn delegation on`がDBへ保存されないのに`ok=true`,`Setting updated`,receipt=`succeeded`を返す一方、panelは同機能を正しく`unavailable`表示する虚偽成功を再現。Critical 0 / Important 1。次はこの1件だけをRED→GREENにし、process/coverage nitへ広げない。新PR・migration・merge・deploy・provider/TG/email/call/L3 side effectは0。
