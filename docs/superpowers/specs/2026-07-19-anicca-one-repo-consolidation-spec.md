...
| 8d.1 | PANEL-0 | permanent personalized dashboard access + connection controls: bookmark可能なstable `/panel`、chat intentとpanel操作を同じuser-scoped commandへ統一。connection card・権限・organ automation・通知・call/委任を本人が接続/切断/ON/OFFできる | RED: fresh token 403、24h固定session、未認証raw 401/403、dead link、non-clickable card、cross-user leakage、hardcoded connection/context、chat/panel state drift。GREEN: 5分単回tokenはlogin bootstrapだけ、token交換後はstable `/panel` + rotating/refreshing HttpOnly session、明示logout/rebind/revoke/storage消去まで通常利用を維持。tenant isolation/CSRF/single-use/auth/action/session-rotation contract 100%。L3: 実TG login→fresh HTTP 200→token消滅→bookmarkした`/panel`をbrowser再起動後も直接再訪→本人固有dashboard。clock-advanced testで24h超のrotation/refreshとlogout/rebind/revokeを実証。harmless toggleをpanel→chat readback→chat intent→panel readbackの双方向で実証し、isolated第2userの表示/action混入0。supported connectorはtest userで実OAuth開始+callback、未提供connectorは正直なunavailable。mobile/desktopで全action clickable | **pending — PR #333 dev merge + exact-SHA staging PASS / production promotion・authenticated L3未了**。PR #332 merge=`835e28a5b668802899b946d56963b4be3366d980`でschema/staging基線PASS。Composio correctiveはRED=`0c3f3df62f46bfb97a4094b75e119a0b85f48452`、GREEN/head=`50f63a20c80ca84136febd27a0b463519e15386b`、PR #333を通常mergeしmerge/origin-dev=`984a088e3ed59941dbb2c1015cef5017ba462d93`。Solとmanagerのfresh primary/focused/full/eval/API/UI=`21/54/328/33/5/6`全PASS。Railway staging deployment=`aa9ec3c2-c448-400d-91eb-1f3f2b1d978d`はexact merge SHAでSUCCESS。manager fresh HTTPはhealth=`200 ok=true`、`/panel=200 no-store / Forbidden=false`、invalid=`403`、control-center/timeline/unauth POST=`401 unauthorized`、receipt/preferences aggregate=`0/0→0/0`。staging evidence=`/Users/anicca/.codex/evidence/panel-0-composio-link-staging.md` SHA-256=`34fafa2536a8a827172278228e5e608285b0c04e785f0b0f6749d1a0671a3f17` mode=`0600`。production/provider/OAuth/env/TG/user preference side effectは0。設定変更TG応答はgeneric `Setting updated`のため値そのもののchat readbackとは扱わず、§9.11 copyを無断変更しない。次はaccepted refsを更新済みの`order-panel-production-l3.md`で`dev→main`通常promotion→production exact-SHA deploy→Dais本人のcontrolled authenticated L3 |
