...
| 8f | CORE-c | context/onboarding/discovery: 初回user、既存user、location未解錠/解錠後、同じclosed Qを二度聞かないことを再検証 | eval 100% + 実TG callback + DB/context provenance。質問禁止領域の発話0件 | **pending — corrective RED/GREENはlocal PASS、commit/release/L3へ**。accepted base=`85a68abaa`、journey RED=`90933cbb9`、corrective endpoint/UI RED/head/upstream=`0fc453527`。corrective REDはserver resume 1、actual client resume/isolation/save/link 4、calendar signature/replay 2の計7件を意図どおりFAIL。minimal GREENは固定context eval=`12/12 (100%)`、実production `/telegram` handlerのcallback/replay/cross-tenant contract、onboarding user-scoped resume、calendar signed connectをPASSし、full Life Call test/evalもfail 0。landing full typecheckの2件は既存scope外baseline（dashboard declaration / income wallet props）のまま。変更はstagedで保全、provider side effect=0。重大事項限定のfresh reviewは実装指摘を返さず10分超で打切り、builder本体も巨大review出力処理から交代。fresh continuation Solがdiff/再検証後にGREEN commit/push→通常PR→exact-SHA deploy→Dais本人の実TG callback・DB/context provenance・semantic duplicate 0・forbidden questions 0をL3実測する | pending |
