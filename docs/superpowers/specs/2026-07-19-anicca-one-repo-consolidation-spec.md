...
| 8 | LM-33 | control panel web UI（§9.9。gpt-tasteskill → frontend-design） | **8a merged + 8b local green、Fable L3/UI 待ち**: LM-33a session を必須にした read-only JSON 5 endpoint（timeline=今日の interpreter 済み calendar + `lm_wake_log`、scores=実集計 + `no_data`、ledger=`lm_api_cost` 集計 + 未実装 FIN 空配列、gates=LM-32 判定/copy、settings=実在する call schedule/接続状態）を実装。request uid は受けず session→uid のみを全 PostgREST/calendar read に束縛し、5 route の session 無し=401、write=405、panel 外 cookie=resource read 0 を fixture test で固定。`npm test` = **293 tests / fail 0 / exit 0**、`npm run smoke:panel-api` = **5/5 HTTP 200 / exit 0**。staging/curl は発注どおり未実施で Fable L3 に委譲。UI 5要素の実ブラウザ表示は次 slice。 | **8b local green・PR/L3/UI 待ち** |
