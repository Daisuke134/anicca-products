...
Incident correction: 2026-07-22、ユーザーの明示指示によりFkimuraの未完成progress artifactをbuyer-visible提出し、`正式な納品`まで送信した。これはINV-R7（合意scope/acceptance完了前はformalを送らない）に反する例外であり、品質gateのgreenとは扱わない。今後の未完成案件はprogress提出のみ、formal checkboxは合意要件/acceptance準備後に限定する。
   ...
   | 3 | **delivery-first loopの自己改善を実証** | 失敗理由をappend-only ledgerへ記録し、次passが同じ失敗を再発させない改善を自動生成・検証する。顧客名・案件IDのハードコードは禁止 | **harness実装・主要E2Eは完了、契約収束は継続。** commits `45d3cbb`〜`2aaa151`で、未完成でもartifact/version/acceptance deltaをbuyer-visible提出、完成時だけformal、post-submitはlive DOMの`納品確認待ち`を正本、未契約inquiryもdaily queue対象にした。ローカルevidenceでlive buyer consent/formal stateを上書きできない。隔離検証はpytest 33 passed / 17 subtests、shell 5/5、CloakBrowser live snapshot 3 orders。証拠 `/private/tmp/gig-runner-todo3-review-2aaa151-20260722/`。現在Fkimuraはformal、sunai/jibieaianはprogress提出済みでbuyer feedback/acceptance待ち、live inquiryは0件。3契約すべてのformal収束は未完了。 |
   | 4 | **provider-agnostic runnerをCapafy/Fleetへ展開** | Provider Registry（Claude Sonnet/Codex Luna/Terra/Sol、capability・quota・transient-only fallback）を共通runnerへ接続 | **Gig production pathは完了、Capafy/Fleet consumer展開は継続。** registry commit `592a193`、stdout quota修正 `ff18e62`。実production E2EでSonnetがweekly limitを返し`transient_quota`、同じschemaをCodex Luna `gpt-5.6-luna`が2試行目で成功。証拠 `/private/tmp/provider-production-e2e-fixed.AjXwm4/`。provider tests 7 passed / 3 subtests。business scriptにprovider/model直書きなし。Capafy/Fleet各consumerの同一runner接続確認はTODO #7で行う。 |
   | 5 | **cleanup control plane + artifact lifecycleを単一化** | cleanup entrypointを1つにし、重複LaunchAgent/OpenClaw cleanerを無効化。全artifactにowner/class/TTL/quota/lease/finalizerを宣言し、fail-closed manifest、off-volume quarantine、append-only delete ledgerを実装 | **完了。** commit `796a7f247`。production deletion authorityは`cleanup_control.py`の1本、schedulerは`com.anicca.emergency-disk-guard`のみ（60秒）、旧`com.anicca.disk-cleaner`/`ai.anicca.disk-janitor`/OpenClaw hourlyはdisabled。manifest欠損/破損/unknown/active lease/deliverable/same-volumeは削除0、期限切れephemeralのみ別filesystemへdigest検証copy後quarantine、append-only JSONL、restoreを実装。pytest 15 passed、guard主要6本・bash/py_compile PASS。deployed guardと正本SHA-256は`cdc0f382afe454d584eafa4f3f04e90a2ab68c106a2215d71d36f2972c9ae2f8`で一致。実機に`/Volumes/AniccaQuarantine`が無いため、現在のproduction sweepは削除せず`quarantine_unavailable`でfail-closed。 |
   | 6 | **producer budgets + capacity observability** | gig / marketing / clip / video / browser / worktree producerにrun quota、rotation、checkpoint圧縮、reserve-space backpressureを実装。容量trendとowner別growthを観測し、0-byte reclaim反復をfailure化 | **共通capacity gate完了。** commit `ef54af2c6`。6 producerを`producer-budgets.json`で明示し、`preflight/complete/rotate`はreserve未満・disk alert・active quotaで新規runをblock、既存active/checkpointを保持、回復後同一runをresume、owner/bytes/free-space trendをappend-only記録、完了runだけrecoverable archiveへrotationする。pytest 22 passed、既存zero-reclaim E2E PASS。production config実行証拠 `/private/tmp/producer-capacity-e2e.J0kzy5/` はGig `started→completed` と容量ledgerを記録。2回連続0-byte reclaimはguardがnonzero+alert+backpressureを維持する。 |
