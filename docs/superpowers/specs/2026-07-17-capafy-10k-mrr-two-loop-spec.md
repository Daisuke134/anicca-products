...
TODO #2C corrective: **validated artifact delivery-only recovery — second-call idempotency active**。commit `82d5f5e`はPAID_WORK green直後にartifact/hash/acceptance/stable evidenceを`artifact_ready_pending_browser=true`として保存し、browser失敗後の次passを`deliver_existing`へ振り分ける。2-pass shell E2Eはpass1 builder→browser fail、pass2 PAID_WORK 0・delivery 1・同一bundle送信を確認し、Planner独立Python 55/55、focused pytest 9、Gig shell 11/11がgreen。runtime 3 blobをidle中にatomic deployし、runs18の実v3 bundleをbrowser-free recoveryすると1回目はstateを`v1 buyer-visible → v3 pending`へ進める。一方、同じrecoveryの2回目はvalidatorのsame-version guardが先に発火し`artifact_version_not_newer_than_project_state`となるため、event重複0でもidempotent success契約は未達。次launchd passは起動せず、同一pending bundleだけを再検証して`reconciled=false / resend=false`で返すcorrectiveをRED→GREEN後に実行する。
