...
| success-state debt | B0/PROFILEのhourly再編集は24h cooldownで解消し、runs=11のpass-reportはexecuted `LEARN/B1/B2/REFLECT` とskipped `B0/PROFILE`を正しく分離した。一方、`.last-pass`は旧lease `gig-95762`・旧funnel値のまま。runs=10のREFLECTは親passを別実行と誤認し、runs=11のB2も新規応募せずsingle-instance確認だけを`ok`返却しているため、success marker・reflection・収益actionの意味はまだ一致しない | commit `93a41b8`、`/Users/anicca/gig/{pass-report.jsonl,.last-pass}`、`/Users/anicca/gig/evidence/gig-pass-{1784694736-13927,1784698024-1204}` |
...
Active TODO #2 gate: **success heartbeat + non-recursive durable reflection**。`.last-pass`は旧lease `gig-95762`本文のままmtimeだけ更新される。REFLECTはruns=8/10で親passを別runnerと誤認し、`gig_pass.sh`再起動→lock回避を`ok`とした。current `PASS_ID` / executed/skipped step / queue evidenceをdeterministic contextとして渡し、reflection中のdriver再入をfail-closedで禁止、結果をschema検証後`reflections.jsonl`へ永続化する。`.last-pass`は同じtop-level success rowをatomic JSONで持ち、pass-reportと不一致ならsuccessにしない。
