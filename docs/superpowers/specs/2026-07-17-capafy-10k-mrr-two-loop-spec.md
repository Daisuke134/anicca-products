...
Active TODO #1 gate: **generic paid work → IFU v3 buyer-visible revision**。runs=14はbuyer feedbackを正しく最優先にしたが、既存PAID_QUEUE_DELIVERYは成果物を作らず、空のagent browser listをCloakBrowser不在と誤認して送信なしで終了した。stable project rootにはv2、source、tests、acceptance、ZIP/hashが存在する。driverは`delivery_project.project_identity`とdelivery-evidence identityを`request_id → talkroom_id → contract_id`へ統一し、project rootをSolのPAID_WORKへ明示する。Solがbuyer feedbackをrequirementsへ保存し、v3 artifact/acceptance/hash/deltaを生成した後だけTerra delivery stepを起動し、CDP `:9222` daily-driverのfresh DOM/screenshotでformalなしのbuyer-visible提出を検証する。agent browser list、`~/Downloads`、顧客名別pathは使わない。
