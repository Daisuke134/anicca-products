...
| success-state debt | B0/PROFILEのhourly再編集は24h cooldownで解消し、runs=11のpass-reportはexecuted `LEARN/B1/B2/REFLECT` とskipped `B0/PROFILE`を正しく分離した。heartbeat/current-pass reflection修正は`1be6628`まで実装済みだがproduction完走は未確認。runs=12は実行中のlive scriptへin-place deployが重なり旧先頭+新後半を読んでparse errorとなったため無効。安定版runs=13ではSunaiの正式納品差し戻しをcollectorが`status=unknown`としてpaid queueから落とし、B1が残存error/freezeのactionable feedbackへ成果物なしの提出予告だけを送り`ok`を返し、そのままB2へ進んだ。success marker・reflection・収益actionの意味はまだ一致しない | commits `93a41b8` / `a18dbc5` / `bfa990f` / `1be6628`、`/Users/anicca/gig/{pass-report.jsonl,.last-pass}`、`/Users/anicca/gig/evidence/gig-pass-{1784694736-13927,1784698024-1204,1784700071-32469,1784700401-52962}`、`/Users/anicca/gig/trajectory/nurture-1784700659/03-returned_delivery_reply_sent.png` |
...
| paid queue | real `received_orders/open` は3件。Fkimuraは完成v2を提出済みでformal `納品確認待ち`。sunai267はv2 formal後に購入者から差し戻され、残存error/freezeの画像2点を受領して`取引中`へ戻った。jibieaianは古い「これからクラファン」前提を終了済みMakuake実績へ訂正し、投稿完成稿2本・carousel 8枚・月5投稿・LINE 5通・導線3件・KPI PDFを含むv2をbuyer-visible提出済み。契約は初月運用中で、購入者承認・Meta/LINE access・公開/計測前のためformalは未送信 | `/Users/anicca/gig/evidence/{fkimura-v2-resubmitted-20260722.png,sunai-v2-formal-message-20260722.png,jibieaian-v2-review-20260722.png}`、`/Users/anicca/gig/trajectory/nurture-1784700659/03-returned_delivery_reply_sent.png`、`projects/{5138597,5167108,17943244}/delivery/v2/submission-evidence.json` |
...
Active TODO #2 gate: **success heartbeat + non-recursive durable reflection + returned-delivery fail-closed**。`.last-pass`は旧lease `gig-95762`本文のまま。current `PASS_ID` / executed/skipped step / queue evidenceをdeterministic contextとして渡し、reflection中のdriver再入をfail-closedで禁止、検証済み結果を`reflections.jsonl`へappendして次LEARNが読む実装は`1be6628`にある。まず安定liveでpass-reportと`.last-pass`が同じtop-level JSONになるproduction E2Eを閉じる。続いてreturned/formal-revision stateをpaidとして分類し、B1後にpaid queueを再評価する。新feedbackに成果物なしの提出予告だけを送った場合はagentの`status=ok`を拒否してfailure ledgerへ記録し、B2を開始しない。顧客名・案件ID・成果物種別はvalidatorへ埋め込まない。
...
|---:|---|---|---|
| 1 | **残り1契約を購入者承認→公開/配信→計測→formal delivery** | jibieaian v2への返信をpollし、投稿ID単位の修正をv3へ反映。承認済み画像/文言だけをMetaへ設定し、LINEは招待受諾→test配信→有効化。公開URL・24h/7d指標をledger/PDFへ記録し、初月合意scope完了時だけ`正式な納品`を付ける | buyer approval message ID、Meta/LINE権限、公開post URL/logged-out screenshot、LINE test/activation、実測KPI PDF、version/hash、buyer-visible formal stateを確認。未承認公開・架空metric・重複進捗文は0 |
...
Current execution: TODO 1のjibieaianはbuyer-review v2提出と自律buyer-wait pollまで完了し、購入者の新返信はまだない。live stateは`取引中` / buyer-visible v2 / formal=false。GPT loopが返信をpollし、承認後にMeta/LINE access→公開/配信→KPI更新→formalの順で閉じる。Sunaiは一度formal `納品確認待ち`となった後に購入者が差し戻し、残存error/freezeの画像を追加した。runs=13のB1はこれを読んだが成果物なしの提出予告だけを送ったためTODO #2のRED実測として扱う。未承認公開、未実測値の0埋め、完成artifactなしの追加テキスト、同一進捗文の重複送信は禁止する。
