...
| Area | 現在の事実 | 一次証拠 |
|---|---|---|
| Claude runtime | direct Sonnet はquota/cooldownで利用不能。共通runnerへ移行済みの経路はClaudeを起動せずGPT attempt 1で成功する。一方、旧persistent coreをhealthcheckが再生成するReddit / Bountyと、Automaton runtimeにはdirect Sonnet processが残る | direct probe、process argv、各healthcheck / runtime entrypoint |
...
Completed gate: **provider-agnostic Gig runner + delivery-first harness — PASS**。corrective commit `61d97b4`、delivery cadence commits `45d3cbb`〜`2aaa151`、provider fixes `592a193` / `ff18e62` は `origin/main` にある。pytest 33件、shell 5/5、provider tests 7件がgreen。production E2EではSonnet weekly limitを`transient_quota`としてCodex Lunaへfallbackする。各契約の成果物完成・再提出・formal deliveryは未完了。証拠 `/private/tmp/gig-runner-todo3-review-2aaa151-20260722/`、`/private/tmp/provider-production-e2e-fixed.AjXwm4/`。

Completed sub-gate: **共通runnerと優先production entrypointのGPT-first化 — PASS**。runner configはLuna/Terra/Sol→Claude fallback、Gig / Capafy marketplace / Capafy IG / clip / connector / reddit daily / life-manager / self-fixのprovider直書きを除去した。focused tests 9件・subtests 17件、consumer wiring 3件、self-fix 13件がgreen。repeatable/tool/high-value live probeと自然Gig reality verifierはCodex attempt 1。commits `d1a51e3` / `551b777` / `58ccec55` / `368dafb7`。旧persistent Reddit / Bounty coreの再生成停止が残るため、全revenue loop gateは未完了。
...
|---:|---|---|---|
| 1 | **稼働中の全revenue loopをGPT-firstで閉じる** | 共通runner・Gig / Capafy / clip / connector / reddit daily / life-manager / self-fixの移行済み変更を保持し、旧persistent coreを再生成するReddit / Bounty healthcheckを共通runner contractへ移す。代替を実測後に旧Sonnet tmuxだけを停止する | quota中のSonnetを起動せず、repeatable/tool/high-value probeがLuna/Terra/Solをattempt 1で選択する。自然Gig passに加えReddit / BountyもGPT provider/model/evidenceを残し、稼働中revenue processのdirect Sonnet argvが0件 |
...
| 6 | **provider-agnostic runnerを残りFleetへ展開する** | Automaton runtime、videoとdormant/manual entrypoint（clip_pass / clip-cli / clip-promote-cli / video-cli / capafy-loop-cli等）のprovider/model直書きを除去する。Bounty / Redditは#1のactive revenue gateで先に閉じる | 対象entrypointのdirect Sonnet argv/文字列が0件。各production pathのattempt 1がGPT既定で成功し、Codex transient failure fixtureではhealth-greenなClaudeへfallback、全provider failureではsuccess markerを更新しない |
