...
Active TODO #1 gate: **generic paid work → IFU v3 buyer-visible revision**。implementation commit `ff45bf6`は`origin/main`へpush済み。stable identityを`request_id → talkroom_id → contract_id`でproject/evidence共通化し、Sol PAID_WORKへstable project rootを渡す。project内のrequirements/source/artifact/acceptance、次の`vN`、acceptance JSON自身のPASS/delta、SHA256をdeterministicに検証し、同じsnapshotをnew evidenceで再queueしてstale blockerを除く。green後だけTerra deliveryを起動し、CloakBrowser CDP health、fresh DOM/screenshot、talkroom URL、添付filename/size、message内version/full hashをbindする。送信後はledgerをcurrent version/hash/buyer-visible/await buyerへ進める。text-only・invalid/stale evidence・同version・empty agent browser list・未解決paidからB1/B2へ進むfixtureはfail-closed。Builder Python 43件、Planner focused pytest 20件+2 subtests、paid-work/returned/delivery-first/buyer-wait/reflection shell E2Eがgreen。launchd/lock/worker idleで既存3fileのpreimageと新規3file absentを確認後、6fileをsame-dir temp+fsync+atomic replaceし、Plannerもlive SHA=`ff45bf6` blob、`bash -n`、`py_compile`を確認した。production runs=15/16はいずれもknown transient `authenticated tab did not finish navigation`がretry後も続きQUEUEでexit 1、mutation/artifact/reply/success marker更新0。直後の同一production CDPへの隔離collectorは16.156秒でrc=0、orders=3 / quotes=0 / inquiries=0を取得した。追加replayはURL一致かつDOM利用可能な`interactive`へ約9秒で到達後も`complete`を待つ旧条件がfalse timeoutを起こすと再現した。commit `b0d7963`は`interactive|complete`を許可し、`loading/about:blank/別path`は拒否する最小修正で、再試行回数は増やさない。Planner独立unittest 46/46、py_compile、diff-checkがgreen。production deployは実行中passを壊さないためidle後に行う。毎時27分の自然runs=17は旧live collectorでも完走し、IFUを`buyer_feedback_or_revision`最優先へ選び、stable project root `/Users/anicca/gig/projects/17943244`でCodex `gpt-5.6-sol` PAID_WORKを起動した。Solはbuyer feedbackをrequirementsへ永続化し、4受入testをRED→GREENへ反転、7.69MBの`artifacts/v3.zip`、acceptance PASS、5点delta、stable delivery manifestを生成した。一時hash不一致も最終化時に解消し、ZIP / acceptance / project manifest / stable evidenceはSHA-256 `d5518fdfb91fb268c6ea2ec7c0589d4ff0dd7b4424ba322c3c0b5f663fa8105e`へ一致。runnerはCodex Sol medium / attempt 1 / `status=success`、queue refresh後はartifact/acceptance/hash blockerが消え、`formal_delivery_not_confirmed`だけが残る。Planner独立検証はZIP integrity、project全12 test、8画像すべて1080×1350 RGB、代表画像のstory-first visualを確認した。Terraは16:49にv3 ZIP 7.3MB・5点delta・full hash・buyer未承認/formal未実施をbuyer-visible送信し、fresh DOM/screenshotでも最新seller messageを確認した。formal checkboxはlive DOMでfalseだが、model生成`paid-queue-evidence.json`に同fieldが欠落し、deterministic validatorが唯一`formal_delivery_checkbox_not_off`で拒否してtop-level exit 1。送信は重複せず、次fixはmanifest contractの明示とdelivery専用self-owned tab+CDP lockでauditor競合を除くこと。一時spec worktreeは`/private/tmp` cleanupで消失したため、remote包含を確認してprotected workspace配下へ復旧した。証拠 `/Users/anicca/gig/evidence/gig-pass-{1784704922-52915,1784705016-56581,1784705225-67113}`、`/Users/anicca/gig/evidence/gig-pass-1784705225-67113/agent-PAID_QUEUE_DELIVERY/{paid-queue-evidence.json,live-dom-17943244.json,talkroom-17943244-post-send.png}`、`/private/tmp/coconala-queue-diag.f13pnX/`、`/tmp/gig-snapshot-afterfix.0BBINb/`、`/private/tmp/ifu-v3-verify.p5x8WB/`。
