...
Active TODO #1 gate: **generic paid work → IFU v3 buyer-visible revision — immediate action PASS / buyer approval・公開・計測・formal pending**。implementation commits `ff45bf6` / `b0d7963` / `ea544c5`は`origin/main`へpush済み。stable identityを`request_id → talkroom_id → contract_id`でproject/evidence共通化し、Sol PAID_WORKへstable project rootを渡す。project内のrequirements/source/artifact/acceptance、次の`vN`、acceptance JSON自身のPASS/delta、SHA256をdeterministicに検証し、同じsnapshotをnew evidenceで再queueしてstale blockerを除く。green後だけTerra deliveryを起動し、CloakBrowser CDP health、fresh DOM/screenshot、talkroom URL、添付filename/size、message内version/full hashをbindする。送信後はledgerをcurrent version/hash/buyer-visible/await buyerへ進める。text-only・invalid/stale evidence・同version・empty agent browser list・未解決paidからB1/B2へ進むfixtureはfail-closed。production runs=15/16のQUEUE false timeoutは、URL一致かつDOM利用可能な`interactive`を許可し、`loading/about:blank/別path`を拒否する`b0d7963`で修正する。自然runs=17はIFUを`buyer_feedback_or_revision`最優先へ選び、stable project root `/Users/anicca/gig/projects/17943244`でCodex `gpt-5.6-sol` PAID_WORKを起動する。Solはbuyer feedbackをrequirementsへ永続化し、4受入testをRED→GREENへ反転、7.69MBの`artifacts/v3.zip`、acceptance PASS、5点delta、stable delivery manifestを生成する。一時hash不一致も最終化時に解消し、ZIP / acceptance / project manifest / stable evidenceはSHA-256 `d5518fdfb91fb268c6ea2ec7c0589d4ff0dd7b4424ba322c3c0b5f663fa8105e`へ一致する。runnerはCodex Sol medium / attempt 1 / `status=success`、Planner独立検証はZIP integrity、project全12 test、8画像すべて1080×1350 RGB、代表画像のstory-first visualを確認する。Terraはv3 ZIP 7.3MB・5点delta・full hash・buyer未承認/formal未実施をbuyer-visible送信し、fresh DOM/screenshotでも最新seller messageを確認する。runs17ではouter manifestのformal field欠落によりtop-levelがfail-closedとなる一方、authoritative live DOMはcheckbox=falseを証明する。`ea544c5`はlive DOM falseを権威化し、true/欠落を拒否、outer fieldをduplicateとして明示する。deliveryはauditorと同じCDP lockを保持し、自分で開いたpersistent default tabだけを操作して必ずcloseする。validator green後だけ動くbrowser-free reconcileは実runs17証拠を再送なしで1回だけ`v2→v3`へ反映し、2回目は`reconciled=false / resend=false`、eventsは12→13、evidence hashは前後不変。reconciled state/eventはanicca-gig commit `91b4032`として`origin/main`へpush済み。Planner独立検証はPython 53/53、Gig shell 11/11、live `bash -n` / `py_compile`、6 runtime blob一致、runs17 validator `ok=true`。残るgateはbuyer approval message、Meta/LINE access、公開/配信、24h/7d実測、formalだけであり、scheduled pollを継続する。一時spec worktreeは`/private/tmp` cleanupで消失したため、remote包含を確認してprotected workspace配下へ復旧する。証拠 `/Users/anicca/gig/evidence/gig-pass-{1784704922-52915,1784705016-56581,1784705225-67113}`、`/Users/anicca/gig/evidence/gig-pass-1784705225-67113/agent-PAID_QUEUE_DELIVERY/{paid-queue-evidence.json,live-dom-17943244.json,talkroom-17943244-post-send.png}`、`/private/tmp/coconala-queue-diag.f13pnX/`、`/tmp/gig-snapshot-afterfix.0BBINb/`、`/private/tmp/ifu-v3-verify.p5x8WB/`。

Active TODO #2 gate: **success heartbeat + non-recursive durable reflection + returned-delivery fail-closed**。`.last-pass`は旧lease `gig-95762`本文のまま。current `PASS_ID` / executed/skipped step / queue evidenceをdeterministic contextとして渡し、reflection中のdriver再入をfail-closedで禁止、検証済み結果を`reflections.jsonl`へappendして次LEARNが読む実装は`1be6628`にある。runs=13はモデルのcurrent evidence path誤記を正しく拒否し、false successを残さないため、このbindingを緩めない。returned/formal-revision stateをpaidとして分類し、未解決feedbackをPAID_QUEUE_DELIVERYへ通してB1/B2を止める。runs18はlaunchd `ai.anicca.hf-gig-pass`からCodex Solを起動し、Sunaiの最新差し戻しを最優先にする。Solはstable project rootで発砲時error/freezeをrequirementsへ保存し、v2修正を保持したv3、acceptance PASS、4点delta、SHA-256 `ec5f901ea6e6f65eb154f9a4cf94855b2f4c2675e054037d9af9113c0a720c7b`を生成する。PlannerはZIP integrity、manifest/stable evidence/hash一致を確認する。続くTerraはshared CDP lockを正しく保持するが、promptのhelper pathが`$HOME/anicca/anicca/...`となり実在pathより`anicca`が1階層多いため安全にdeferし、browser送信0、validator failure、top-level exit 1、lock解放となる。commit `42d54b6`はpathを正本`$HOME/anicca/skills/browser/scripts/cdp_default_tab.py`へ修正し、open/close各1回、double path 0、自所有target/ws限定をfixture化する。新feedbackに成果物なしの提出予告だけを送る、または過去passのvalid artifact/evidenceを再利用する場合も成功にしない。commit `7a2ea28`はcurrent pass開始時刻をvalidatorへ渡し、requirements/acceptance/project manifest/stable delivery evidenceのstale suiteを拒否する。artifactはmanifest SHAでbindし、大容量repack互換を保つ。Planner独立Python 54/54、Gig shell 11/11、py_compile/bash-nがgreenで、両commitのruntime blobをlaunchd/lock idle中にatomic deployする。次passは既存Sunai v3をbuyer-visible進捗提出し、正式納品を付けず実機feedback待ちへ進める。残るreflection subgateはstructured current-pass field、証拠実体/freshness、legacy heartbeat/競合rollback、再帰拒否時の3 ledger無変更をRED化する。顧客名・案件ID・成果物種別はvalidatorへ埋め込まない。証拠 `/Users/anicca/gig/evidence/gig-pass-1784708164-17503`。

TODO #2C corrective: **validated artifact delivery-only recovery — second-call idempotency PASS**。commit `82d5f5e`はPAID_WORK green直後にartifact/hash/acceptance/stable evidenceを`artifact_ready_pending_browser=true`として保存し、browser失敗後の次passを`deliver_existing`へ振り分ける。commit `efc3c3c`は同一versionを無条件に許可せず、current pending marker・manifest/state/stable evidence/artifact/hash/acceptance/path/mtimeが完全一致する場合だけfull validationを再実行して`reconciled=false / resend=false`を返す。Planner独立検証はPython 55/55、focused pytest 9、Gig shell 11/11、pycompile green。runtimeへexact blobをatomic deploy後、runs18の実Sunai v3 bundleでproduction `--mark-ready`を連続2回実行し、両方`ok=true / reconciled=false / resend=false`、event count `19 → 19`、stateは`current_version=v3`、`current_package_sha256=ec5f901ea6e6f65eb154f9a4cf94855b2f4c2675e054037d9af9113c0a720c7b`、`buyer_visible=false`、`artifact_ready_pending_browser=true`、`next_action=retry_buyer_visible_delivery`を維持した。次は実launchd passでSolを再実行せず、同一bundleをTerraがbuyer-visible progress提出するE2Eを取得する。

TODO #2C production run 19: **buyer-visible send PASS / harness reconciliation FAIL**。実launchd `ai.anicca.hf-gig-pass` runs=19、evidence `/Users/anicca/gig/evidence/gig-pass-1784710595-93149` はSunai `request_id=5167108` / `talkroom_id=18011694`を選択し、`agent-PAID_WORK`を作らずTerra `gpt-5.6-terra`の`PAID_QUEUE_DELIVERY`だけを実行した。CloakBrowser実画面で18:01に`v3.zip` 33,886,664 bytes、SHA-256 `ec5f901ea6e6f65eb154f9a4cf94855b2f4c2675e054037d9af9113c0a720c7b`、acceptance delta 4件、未実施のMinecraft実機確認を明記してbuyer-visible送信し、正式納品checkbox OFFをscreenshot SHA `a10d78b8ba6c3f80dd323a776b43cbf01fa21007f5941b24735398a9e4cf7a53`とlive DOM SHA `6d7cab08185f5f0e567064c90348c9e4723ca0a0b4e5a1e1ef0cd1309af7a4c1`でPlannerが独立確認した。専用tabとCDP/pass lockは終了時に消滅。ただしprompt/agentはlive DOM URLを`talkroom_url`で保存しvalidatorは`url`を要求したため、`post_send_live_dom_url_missing` / `post_send_url_not_bound_to_queue`でexit 1、stateは`buyer_visible=false / artifact_ready_pending_browser=true`のまま、event countはqueue_selected分だけ`19 → 20`。buyerへの実送信は重複させず、URL field契約を既存互換のまま単一化して既存run19証拠を再検証・reconcileするcorrectiveが次gate。

TODO #2C run 19 reconciliation corrective: **production PASS**。commit `9257b42`（test補強 `c2a52df`）は新規promptのlive DOMをcanonical `url`へ固定し、validatorは既存`url`を維持したままpre-existing `talkroom_url` aliasを読み、両field競合・別talkroomをfail closedにする。Planner独立検証はunittest 56/56、pytest 70 + subtests 25、Gig shell 11/11、pycompile、diff-check green。Gig/Auditor idle・CDP/pass lock不在時にruntime 2 blobをexact deployし、run19の実evidence + queue manifestをread-only validatorへ渡すと`ok=true / errors=[]`。browser-free reconcile 1回目だけevent `20 → 21`を追加して`buyer_visible=true / artifact_ready_pending_browser=false / latest_buyer_visible_version=v3 / next_action=await_buyer_feedback`へ進み、2回目は`reconciled=false / resend=false`かつevent `21 → 21`。buyerへのbrowser再送は0で、Sunai v3 delivery-only recoveryの実提出と内部台帳が一致した。

TODO #2C durable project ledger: **PASS**。clean `anicca-gig` worktreeからcommit `db1c3d37c7b03974954cdc65da05f82f68146f51`を`origin/main`へpushし、Sunai v3のrequirements/source/tests/acceptance/artifact/delivery manifest/stable evidence/state/eventsを固定した。remoteのv2正式納品履歴5件を上書きせず、live側の新規10 eventだけをappendして全26 eventとし、`formal_delivery_sent`、`paid_work_ready_for_browser`、`paid_work_browser_sent_reconciled`を同時に保持。artifact ZIP integrity、project unittest 2/2、full reference `613/613` + particle `8/8`、artifact/addon/manifest/stable evidence hash一致を再検証した。

TODO #2 reflection/heartbeat corrective: **code + fixture PASS / production run 21 RED**。commit `7b5da70`はREFLECT resultのfree-form bindingを廃止してstructured `current_pass`（pass_id、resolved queue/evidence path、executed/skipped steps）を必須化し、queue/summary/result/evidence directoryの実体とcurrent-pass freshnessを検証する。finalizeはfcntl lock下でpass-report appendとheartbeat atomic replaceを行い、legacy plaintext heartbeatをcanonical JSONへ置換する。heartbeat失敗時は他writerのsuffix/heartbeatを壊さず自pass rowだけを除き、直前のreflection rowもrollbackする。`GIG_REFLECT_ACTIVE=1`再帰はlock/browser前rc97で3台帳無変更。Planner独立検証はunittest 56/56、pytest 70 + subtests 25、Gig shell 11/11、bash-n、diff-check green。pre-deployの自然run 20はIFU buyer-wait→Luna LEARN→Terra B1/B2→Luna REFLECTまで進み、最後のself-improve verifier failureでsuccess heartbeatを残さずexit 1。原因は`applied.jsonl`の1物理行にliteral `\\n`で2 JSON objectが連結された既存malformed rowを`apply_volume`算出がblind `json.loads`したこと。commit `dbf6c33`はledger自体をrewriteせず、malformed/non-object行をfail-closed skipし、valid rowだけで算出してline/error/bounded previewをTODO/audit/stdoutへ残す。Planner独立検証はunittest 57/57、pytest 71 + subtests 25、Gig shell 11/11、bash-n、diff-check green。reflection runtimeをcommit blob `00293d5c1d67ff4543139f8f49d7f4d6c1e6a28d`へexact deployした後のrun 21は、Luna LEARN、Terra B1/B2、Luna REFLECT agent実行自体は成功したが、REFLECT resultが必須structured `current_pass`を返さずvalidatorが拒否した。top-levelは`failed_step=REFLECT` / `reason=reflect_validation_or_ledger_failed`でexitし、run 21のreflection/pass-report/heartbeatを更新していない。根因は全step共通schemaが`additionalProperties:false`かつ`status/summary/evidence`しか許可せず、prompt必須の`current_pass`をschema上出力不能だったこと。commit `3b57960`はREFLECT専用strict schemaを追加し、5 fieldをrequired、nested追加fieldを拒否し、他stepのschemaとvalidatorは変更しない。Planner独立検証はunittest 57/57、pytest 71 + subtests 25、Gig shell 11/11、Draft 2020-12 schema、bash-n、diff-check green。次はidle中に`3b57960`の`gig_pass.sh`+専用schemaと`dbf6c33` verifierをatomic deployし、run 22で同一pass整合、malformed diagnostic、top-level exit 0を確認する。証拠 `/Users/anicca/gig/evidence/gig-pass-1784713290-83173`、`/Users/anicca/gig/pass-failures.jsonl`。
...
TODO #2 current production update: **Sunai v4 buyer-visible + reflection/heartbeat production proof PASS / external formal gates remain**。run 22のSol PAID_WORKは従来validatorが見落としていた到達可能431 animationのsound timeline 929参照中の未定義10件とplayer sound mapping 642件中の未定義1件をRED化して修正し、v4を生成した。artifact SHA-256は`200e7ed91ba0ba0bca1a5853871b98d81c4530d005934ba9253d0936de52c020`、Minecraft実機確認は未実施と明記する。v4 project ledgerはcommit `1f07bff1`としてanicca-gig origin/mainへpushし、v3履歴をprefix保持、project pytest 7/7とmissing sound 0をPlannerも再検証した。run 22 deliveryはself-owned CDP lockを競合と誤認してRED。commit `52066bb2`はwrapperがagentを起動できた時点でこのstep自身のleaseであり、agentはlockを再判定せず、真の競合ではagent未起動となる契約をfixture化する。Planner独立検証はunittest 57/57、pytest 71 + subtests 25、Gig shell 11/11、bash-n、py_compile、diff-check green。runtime blob `4f84bf29aafc36935373c849d3cd3629992e8b8b`へexact deployしたrun 23はTerra delivery-onlyで21:26にv4.zip 33,863,502 bytesと完全SHAをbuyer-visible送信し、formal checkbox OFF、canonical URL、attachment/version/hash/delta、post-send screenshot/live DOMをvalidatorが受理した。stateは`current_version=v4 / buyer_visible=true / artifact_ready_pending_browser=false / latest_buyer_visible_version=v4 / next_action=await_buyer_feedback`、top-levelは正式承認待ちを`paid_delivery_progress_sent_formal_pending`としてexit 1。clean worktreeでremote v4 ledgerへ2 eventと3証拠だけを移植したcommit `9015ecb`はanicca-gig `origin/main`と一致し、3証拠SHA、queue-bound validator、project pytest 7/7をPlannerも再検証した。直後の自然launchd run 24（pass `1784723225-67429`）はIFU buyer-waitをmutationなしでpollし、Codex Luna LEARN、Terra B1/B2、Luna REFLECTをattempt 1で実行する。B1は新しい購入案件`5164802`を検出し、必要な原本再添付をbuyer-visible依頼した。REFLECTは専用strict schemaの`current_pass`へ同じpass ID、resolved queue/evidence path、executed `LEARN/B1/B2`、cooldown skip `B0/PROFILE`を返し、`reflections.jsonl`へappendした。pass-report最終行と`.last-pass`はbyte一致し、`applied.jsonl`物理191行目の連結JSONはrewriteせず`malformed_count=1`とbounded previewをself-improve TODO/auditへ記録した。launchdは`last exit code = 0`、pass/CDP/delivery lockは全て不在。証拠 `/Users/anicca/gig/evidence/gig-pass-1784722969-49724` と `/Users/anicca/gig/evidence/gig-pass-1784723225-67429`。残るTODO #2 gateは3契約の外部承認後formal状態と、次passで今回のreflection/diagnosticをLEARNが実際に利用する継続証拠。

TODO #2 continuation run 25: **previous-reflection consumption PASS / bounded-step ownership RED**。launchd kickstart pass `1784723805-18782`のLuna LEARNは`reflections.jsonl`最終行を実読し、run 24 pass `1784723225-67429`のformal未確認状態をlessonへ変換、requestId+outcome重複確認後にGitHub issue `#1070`へ共有した。Terra B1は最新seller messageを確認して重複送信0、Terra B2は実UIの有償3件（Sunai要修正、IFU取引中、Fkimura納品確認待ち）を理由に新規応募・出品変更を0件とした。Luna REFLECTは同一pass bindingを返し、最終pass-reportと`.last-pass`はSHA `711532cc...`でbyte一致、launchd exit 0、全lock不在。ただしB1/B2の共通promptがfull-pass `GIG_PASS_RUNBOOK.md`を読むため、bounded actionを終えた後もorchestrator-owned処理を実行した。B1は`applied.jsonl`物理191行目をappendではなくin-place patchして193→194行、85,919→85,918 bytes、SHA `949c9067...`→`f420c075...`へ変更し、最終malformed diagnosticを不正に1→0へ消した。さらに`playbook.json`と`gig-funnel.jsonl`を更新し、実buyer action 0にもかかわらずlegacy pass `p-1782860280 / nurture_actions=1`を`pass-report.jsonl`へappend、run24本文の`.last-pass`を中間touchした。B2は`gig_single_instance.sh`、`passprep.py`、browser/lock setupを再実行し、`strategy.pass_count`を進めた。最終wrapperがcanonical heartbeatを上書きしたため外形上greenだが、中間crash時にはfalse heartbeatと重複orchestrationを残し得るため完了不可。REFLECTのstructured `current_pass`は正しい一方、descriptive evidenceに存在しない`gig-pass-1783805-18782/...`を1件返しており、local evidence path実在性も追加RED。次gateはstep共通契約を分離し、LEARN/B1/B2がwrapper-owned single-instance/passprep/finalize/heartbeat、他step、既存JSONL rewriteを実行不能にするRED fixture→修正→自然launchd E2E。その後、REFLECTのlocal evidence path実在性を別gateでfail-closedにする。証拠 `/Users/anicca/gig/evidence/gig-pass-1784723805-18782`。

TODO #2 bounded-step corrective + run 26: **code/fixture PASS / paid-feedback delivery PASS / boundary production gate INCOMPLETE**。commit `8f01433110b7dbd02fecc7aab871db7153d033a9`はbounded child promptからfull-pass `GIG_PASS_RUNBOOK.md`を除き、LEARN/B0/PROFILE/B1/B2/REFLECTごとのallowed/forbidden contractを直接埋め込む。childは`gig_single_instance.sh`、`passprep.py`、他step、funnel/self-improve、pass-report/heartbeat/finalize、既存JSONL rewrite/truncateを実行せず、明示許可されたaction rowだけをappendできる。B1/B2はparent pass lockを触らず`${GIG_LEASE}-B1/-B2`のstep-owned browser contextと自己所有tabだけをacquire→finally releaseできる。Planner独立再検証はfocused contract、Node 72/72、bash-n、remote hash一致がgreen、Builder full gateはunittest 57/57、pytest 71 + subtests 25、Gig shell 11/11、Node 72/72、全Gig bash-n/pycompile/diff-check green。commit blobをidle中のruntimeへexact deployしたlaunchd run 26（pass `1784725326-22824`）は、境界step前にSunaiの新buyer feedback「これは、mcaddon 化してほしいですね。」をlive検出した。Sol PAID_WORKはv4.zipの外装を除いた直接import用`v5.mcaddon` 37,767,705 bytes、SHA-256 `70ad38ad17cd94481616384bf16a286ac50b227255ebc33fc7dbd3242b367475`を生成し、8/8 regression、pack/alias/particle/sound参照を再検証した。Terra PAID_QUEUE_DELIVERYは22:09にmessage ID `js-talkroomMessage-217987495`としてv5をbuyer-visible送信し、live DOM・screenshot・queue-bound validatorはattachment/version/full hash、formal checkbox OFFを確認した。これはrun開始時に未処理だった新要望への1回送信で重複ではない。設計どおり`paid_delivery_progress_sent_formal_pending`でexit 1し、LEARN/B1/B2/REFLECTへ到達しなかった。`applied.jsonl`、pass-report、`.last-pass`、strategy `pass_count=516`、playbook、reflections、shared-lessonsはrun前後でSHA/mtime不変、終了時worker 0・全runtime lock不在。よってpaid-feedback deliveryはgreenだがbounded-step production gateは未評価。次はseller v5送信後・buyer未返信の状態でrun 27を起動し、LEARN/B1/B2/REFLECTだけが各ownership内で完走することを実測する。その後REFLECT local evidence path実在性を別gateでfail-closedにする。証拠 `/Users/anicca/gig/evidence/gig-pass-1784725326-22824`。

TODO #2 run 27: **bounded-step ownership production PASS / B1 route + B2 paid-gate RED**。launchd pass `1784726075-67832`はIFU buyer-waitをmutationなしでpollし、parent `passprep.py`を1回だけ実行して`strategy.pass_count 516→517`、Luna LEARN、Terra B1/B2、Luna REFLECTを全てattempt 1で完走、B0/PROFILEをcooldown skip、exit 0となった。全4 promptでfull `GIG_PASS_RUNBOOK.md`参照0、ownership/forbidden contractあり。child command実測ではgig_pass/launchd/single-instance/passprep/他step/funnel/self-improve/finalize/heartbeat再実行0、既存JSONL rewrite/truncate 0。`applied.jsonl`とplaybook/shared-lessonsはSHA不変、pass-reportはwrapper canonical row 1件だけappend、`.last-pass`は中間touchなしで最後にinode swapしcanonical rowとbyte一致、B1/B2のstep-owned leaseは各1回acquire/releaseして残存0、REFLECTのcurrent-pass bindingと今回列挙したlocal pathは全て実在した。よってrun25の責務汚染はproductionで再発せず、このsubgateはgreen。ただしB1は実在しない`/mypage/messages`を開いて404を成功扱いし、既知talkroomをsweepしなかった。B2はcurrent `delivery-queue.json` / marketplace snapshotを読まず、有償3件（Sunai/IFU未完了、Fkimura formal pending）が残るのにrequest `5170046`を候補化して「応募する」を2回clickした。フォームが開かず実応募・台帳追記0だっただけで、paid-gate enforcementの結果ではないためfunctional gateはRED。次はB1へdeterministic snapshot/talkroom queueを明示し、B2はpaid/feedback/proposal queueが非emptyならagentを起動せずwrapperでskipするRED fixture→修正→自然launchd E2Eを先に行う。その後REFLECT local evidence path実在性をvalidatorでfail-closedにする。Sunai v5 project/evidenceはclean worktree commit `3c2623b3ba687a803e86c0897c0ff098a15537c5`としてanicca-gig `origin/main`へpushし、remote 30 event prefixを保持してv5 event 2件だけappend、project pytest 10/10、queue-bound validator、remote artifact/evidence hashをPlannerも再検証した。証拠 `/Users/anicca/gig/evidence/gig-pass-1784726075-67832`。

TODO #2 B2 deterministic paid gate + runs 28/29: **production PASS / B1 deterministic route remains**。commit `f55539be5dab3e2d5ce7d876fcbec47a359a0b94`はcurrent delivery queueをagent起動前に検査し、`overdue_or_today_paid_deliverable` / `buyer_feedback_or_revision` / `quote_needing_proposal` / `other_paid_work`のいずれかが未解決ならB2をskipする。known lower-priority class `nurture` / `listing_apply_learn`だけを通し、missing/malformed/unknown queueはrc 2でpassをfail-closedにする。skip理由はREFLECT strict schema、reflection ledger、pass-report、`.last-pass`へ同じ値で永続化する。Builder full gateはunittest 60/60、pytest 74 + subtests 34、Gig shell 11/11、Node 72/72、全Gig bash-n/pycompile green、Planner focused testもgreen。commit blobをidle中のruntimeへexact deployした。deploy前の自然run 28（pass `1784726825-20588`）ではLuna/Terra自身が前回失敗から正しい`https://coconala.com/message?fromMyPage=true`へ到達し、3会話を確認してgrounded replyなし、B2もpending deliveryを認識して応募0、REFLECT/exit 0となったが、これはmodel判断でwrapper enforcementの証明ではない。deploy後のlaunchd run 29（pass `1784728208-12147`）はqueue SHA `5910e65b...`の`other_paid_work / talkroom 17943244 / paid / formal=false / buyer_agreement=false / delivery_ready=false`を読み、gateを2回実行していずれもrc 0・同一理由`B2:unresolved_higher_priority_queue:other_paid_work:17943244`を返した。B2 prompt/agent/runner/browser context/leaseは生成0、`applied.jsonl`・action map・playbook・shared-lessonsはSHA/mtime不変。Luna LEARN、Terra B1、Luna REFLECTはattempt 1で成功し、B1の送信0、`strategy.pass_count 518→519`、pass-report 1行・funnel 1行・reflection 1行だけがexpected deltaとなった。REFLECT `current_pass`、final pass-report、`.last-pass`は同じskip理由を持ち、後二者はSHA `2144dc629ba69690602682a135a83be64917fc81960ac3fd86b155bee7f66a55`でbyte一致。launchd runs=8 / exit 0、全pass/CDP/delivery lockとcurrent leaseは不在。B2 subgateは完了。次gateはB1へcurrent snapshotと正しい受信箱/talkroom queueをdeterministic入力し、404 routeを試行せず全actionable conversationをsweepするRED fixture→修正→自然launchd E2E。その後REFLECT local evidence path実在性を別gateでfail-closedにする。証拠 `/Users/anicca/gig/evidence/gig-pass-{1784726825-20588,1784728208-12147}`。

TODO #2 B1 deterministic route code + run 30: **code/fixture PASS / paid revision+buyer-visible v6 PASS / progress finalize RED / B1 production preempted**。commit `5bad6398bb18bdcdecbec216217c76441e6cbbd6`はcollectorの旧404 `/mypage/messages`を正規`https://coconala.com/message?fromMyPage=true`へ置換し、query消失・route mismatch・404 titleをfail-closedにする。wrapperはcurrent marketplace snapshotとdelivery queueからopen order/inquiry/queue talkroomを顧客名・ID hardcodeなしでunionし、B1専用strict schema/validatorへinboxと全talkroomのfresh owned screenshot/live DOM exact coverageを要求する。status-only成功、talkroom欠落/追加、別URL、stale/escaped evidenceは拒否し、返信内容/送信判断/action ledger semanticsは変更しない。Builder full gateはunittest 65/65、pytest 79 + subtests 42、Gig shell 12/12、Node 72/72、全Gig bash-n/pycompile/diff-check green、Planner focused 21+shellと4 runtime blob byte一致もgreen。launchd run 30（pass `1784730220-40333`）は正規inbox `not_found=false`と3 talkroomのsnapshot/evidenceを取得した直後、Sunaiのv5後buyer feedback「やはり、HUDのバグと瞬間的なフリーズは残っていますね。。。」を検出したため、B1よりpaid revisionを正しくpreemptした。Sol PAID_WORKはHUD namespace欠落、毎tick 39関数無条件実行、未定義`m107` normal/empty item参照を特定し、HUD dispatcherをplayer/item scopeの1 entrypointへ縮約した`v6.mcaddon` 37,768,258 bytes、SHA-256 `b3046d0930fa4c2c8828a465900c99ac0045a5ee980354c106364e69ed185dc7`を生成する。project unittest 11/11、ZIP CRC、known 14参照、到達可能animation/controller 613、particle 8、sound timeline 929、player sound 642、missing 0、acceptance/delivery/stable manifest/hash一致がgreen。Terraは23:33にv6とfull hash/deltaをbuyer-visible送信し、fresh DOM/screenshotで新規seller message 1件、attachment 1件、duplicate 0、formal checkbox OFFを確認する。B1/B2 agentは未起動、全runtime lock/lease/processはcleanup済み。ただし`gig_pass.sh`はvalid progress送信後も`formal_delivery_not_confirmed`を無条件にfailure ledgerへ記録してexit 1するため、run30のREFLECT/reflection/pass-report/`.last-pass`は未生成、最後のheartbeatはrun29のままとなった。次gateはprogress提出成功とformal完了を分離し、未解決paid中のB1/B2はpolicy skipしたままREFLECT/finalizeを成功させるRED fixture→修正→launchd E2E。その後、外部buyer待ちpassでB1 deterministic inbox/talkroom coverageをproduction実証し、REFLECT local evidence path実在性を別gateでfail-closedにする。証拠 `/Users/anicca/gig/evidence/gig-pass-1784730220-40333`、baseline `/tmp/gig-run30-verifier-baseline-1784730195.txt`。

TODO #2 run 30 durable v6 ledger: **PASS**。v6のproject/acceptance/artifact/delivery manifest/stable evidence/state/eventsはclean worktreeからcommit `d1ac248b9b5a7b177b4d4404b75037a199471171`としてanicca-gig `origin/main`へpush済み。Plannerはremote exact、artifact SHA-256 `b3046d0930fa4c2c8828a465900c99ac0045a5ee980354c106364e69ed185dc7`、project unittest 13/13、acceptance PASS、buyer-visible attachment/hash/formal checkbox OFF、既存events byte-prefix保持を独立再検証した。live dirty treeのstage/restore/cleanは行っていない。

TODO #2 progress-finalize corrective: **code/fixture PASS / production E2E running**。commit `64a725cb3ca7844a5e7387ef74514ce22072abdf`はqueue-bound browser evidence validatorが成功した後、`delivery_action=progress`かつ唯一のblockerが`formal_delivery_not_confirmed`の場合だけをvalidated progress/formal pendingに分類する。PAID_WORK/PAID_QUEUE_DELIVERYをexecutedとし、formal完了・earnedを立てず、LEARN/B0/PROFILE/B1/B2とpassprep/funnel/self-improveをskipし、REFLECTとcanonical finalizeでexit 0とする。unexpected blocker/malformedは従来どおりfail-closed。Builder full Gig regressionはunittest 69/69、pytest 83 + subtests 48、Gig shell 12/12、Node 72/72、bash-n 29、pycompile 49がgreen。Planner独立検証はunit 17/17、3 shell gate、bash-n/py_compile、remote exactがgreen。launchd idle・全runtime lock不在中に`gig_pass.sh` blob `295ddd22fa2aa04776df4db6db55ba58c2c18d15`と新gate blob `a9d53d9df61221b1e3944afdc53a80eac1758985`のみをruntimeへexact deployした。既送信v6は再送せず、次の自然launchd passでproduction検証する。

TODO #2 run 31: **B1 deterministic collection PASS / agent provider RED**。stale preconditionとして2026-07-15から残った空の`~/gig/.cdp-guard.lock`を検出した。owner/lsof/process 0、CDP `:9222` HTTP 200、healthy daily-driver rootを実測後、空の正確targetだけを`rmdir`して全lock不在を回復した。launchd run 31、pass `1784732195-96376`はcanonical inboxと3 open talkroom `17943244/17963099/18011694`のexact B1 context、fresh owned JSON/PNG、ID/URL/screenshot SHA一致までgreen。buyer-visible mutation/duplicateは0。ただしLEARNのcodex `gpt-5.6-luna`はusage limit、Jul 29 02:02再開表示でrc 1、fallback Claudeはlaunchd PATH上binary不在rc 127で、B1/B2/REFLECT未実行、launchd runs `9→10` / exit 1。`strategy.pass_count 519→520`とfailure row 1件以外の下位台帳はSHA/行数不変、全lock/worker cleanupはgreen。direct Claudeもweekly limit Jul 24 06:00、OpenClaw `openai/gpt-5.4-mini`はcooldown、Moonshot KimiとxAIはbilling errorで、実probeで`openclaw agent --model blockrun/free --thinking off --json`だけがstrict JSONを返した。次gateはGPT-first候補を維持し、一時quota/unavailable時だけOpenClaw/blockrun-freeへ切り替えるprovider adapterをRED fixture→実装→同じbuyer-wait production passでB1/B2 skip/REFLECT/exit 0まで実証する。証拠 `/Users/anicca/gig/evidence/gig-pass-1784732195-96376`、baseline `/tmp/gig-run31-baseline.jsonl`。

TODO #2 run 31 provider corrective: **code/fixture PASS / production run 32 running**。commit `4f4e9ccc20d11c7d674b72ed8f2fcc3a06756a75`は全agent task classの候補順をCodex GPT-first→Claude Sonnet→OpenClaw `blockrun/free` last-resortとし、high-valueはCodex medium/highの後にSonnet/freeを続ける。ClaudeはPATH優先、不在時にuser-relative `~/.local/bin/claude`をHOME展開して実行可能な場合だけ使う。OpenClaw adapterはunique session、agent/model/timeout、strict full schema/workdir/JSON-only suffix、wrapper `result.payloads[0].text`のfresh result抽出、既存schema validation、durable provider/model/executable/session/adapter evidenceを持つ。timeout/quota/unavailableのみ次候補へ進み、nontransient task failure、malformed/missing payload、rc0 invalid schemaはfail-closedで停止する。Builder full Gig regressionはunittest 75/75、pytest 92 + subtests 52、Gig shell 12/12、Node 72/72、bash-n 29、pycompile 52がgreen。Planner独立検証はunittest 13/13、candidate順/portable resolver、py_compile、remote exactがgreen。launchd idle/lock不在中にruntimeの`agent_runner.py` blob `b7ea4bf54ab3b0f89273fec3cdffa02f1dd7e5ae`と`config.json` blob `de15124d060877beedd275cbf3c9897b5933e01d`だけをcommitとexact一致させた。次の自然launchd run 32でCodex/Claude quotaからfree fallback、B1/B2 skip/REFLECT/canonical exit 0まで実証する。

TODO #2 run 32: **fresh buyer feedback routing PASS / OpenClaw fallback RED**。launchd pass `1784733439-86491`はv6後の新規buyer返信をlive検出した。実画面は7月23日00:06に銃のモーション・発射・右下残弾HUDの喪失、HUD errorと瞬間freeze継続、00:08に「構えるだけで、reloadしかしない銃」との補足、formal差し戻し通知を示す。よって`buyer_feedback_or_revision`が最優先となりPAID_WORKへ入ったのは正しい。Codex Sol medium/highはどちらもusage limitを`transient_quota`として次へ進み、Claudeはportable resolverで`/Users/anicca/.local/bin/claude`を実行できた。しかしstdoutの`Failed to authenticate: OAuth session expired and could not be refreshed`を`validation_or_task_failure`と誤分類し、第4候補OpenClawへ進まずexit 1。v7 artifact、buyer-visible mutation、duplicateはすべて0、v6 artifact/hash/delivery evidenceは不変。3 talkroomのfresh JSON/PNG exact coverage、下位台帳不変、failure row 1件だけappend、全lock/worker cleanupはgreen。次gateはexpired OAuth/session refresh failureのみを一般化したtransient auth/unavailableに分類し、真のnontransient task/permission/invalid credential failureはfallbackさせないRED fixture→修正→自然pass。OpenClawが実v7を生成する場合も既存artifact/acceptance/hash/freshness validatorとbuyer-visible queue-bound validatorを緩めない。証拠 `/Users/anicca/gig/evidence/gig-pass-1784733439-86491`、baseline `/tmp/gig-run32-baseline.jsonl`。

TODO #2 run 32 auth corrective: **code/fixture PASS / production run 33 RED**。commit `8028213e6e14fb7082d2aa1eaa59e2b8a5323fba`はOAuth/auth/session/access tokenのexpiredまたはrefresh failureの明示的な文言だけを`transient_auth`に分類する。invalid credentials/API key/token、permission denied/insufficient permission/forbidden/unauthorizedはnontransientとして停止する。Builder full Gig regressionはunittest 77/77、pytest 94 + subtests 59、Gig shell 12/12、Node 72/72、bash-n 29、pycompile 52がgreen。Planner独立検証はagent-runner 15/15、py_compile、remote exactがgreen。launchd idle/lock不在中にruntime `agent_runner.py` blob `8adf18ac92279a68fe11900f31c4f443f6bc0155`をexact deployした。

TODO #2 run 33: **provider fallback PASS / real artifact production RED / fail-closed PASS**。手動kickしたpass `1784734023-35338`は00:27の実`StartCalendarInterval`発火時にSIGTERMで停止し、自然launchd run `1784734086-38155`へ単一workerとして交代した。自然runはCodex Sol medium/highのusage limitを`transient_quota`、Claude Sonnetのexpired OAuth refreshを`transient_auth`として扱い、第4候補OpenClaw `blockrun/free`まで自動継続した。OpenClaw wrapperはrc 0、schema valid、`status=ok`を返したが、主張した`minecraft-gun-addon_v20260723-0129.mcaddon`と`acceptance-v6.5.json`を実際には作らず、requirements/delivery evidenceもv6のままだった。既存validatorは`manifest_stale`、`requirements_stale`、`acceptance_stale`、`artifact_version_not_newer_than_project_state`、`delivery_evidence_stale`の5項目で拒否し、PAID_QUEUE_DELIVERY/ブラウザ送信へ進まずlaunchd exit 1、buyer-visible mutation/duplicate 0、v6 state/artifact/hash不変。よってfree fallbackはagent harnessの接続証拠にはなるが、この高難度Minecraft修正のproduction providerとしては未達。次gateはvalidatorを緩めず、実ファイルを書けるprovider/能力だけをhigh-value候補として選別するcapability proofを追加し、利用可能なGPT providerでv7 real artifact→fresh acceptance/manifest→queue-bound進捗提出→formal OFF→progress-finalize exit 0まで再実証する。証拠 `/Users/anicca/gig/evidence/gig-pass-1784734086-38155`。

TODO #2 run 34 capability corrective: **external tool-write proof / code / regression PASS、production RED**。外部mktemp nonce read→exact output write→readback probeでは`blockrun/free/qwen3-coder-480b`と`blockrun/free/deepseek-v4-flash`がrc 0の成功文だけを返し実ファイル/tool callが無いため失格、`blockrun/free/gpt-oss-120b`だけがwrapper `toolSummary calls=3 / tools=[read,write] / failures=0`、input/output SHA-256 `f29f21aabb749d09c5a905e263331922a80b86d0a9638e513e2e9728b5aada30`一致を満たした。commit `87d17d38b4b1ad0fd2ddf76e435449abbd5fba7a`はhigh-valueの最終候補をこの明示GPT-OSSへ置換し、candidate `required_capabilities=[tool_write]`をprovider/model exact capability registryへ照合して、未証明・malformed時はprovider起動前rc 2でfail-closed evidenceを残す。repeatable/tool candidate順、task failure stop契約、artifact/browser validatorは不変。Builder regressionはunittest 79、pytest 96 + subtests 59、Gig shell 12、Node 72、bash-n 29、pycompile 52がgreen。Planner独立検証はagent-runner 17/17、remote exact、runtime `agent_runner.py` blob `6800c8f29944ea1793cb32d276e320d3bbc9e843` / `config.json` blob `f485bd04988dc3887f8492b65df49b1b857fecb5` exact deployがgreen。

TODO #2 run 34 production: **small probe capability does not imply per-run capability / RED**。launchd run 14、pass `1784735039-10703`はsingle workerで同じfresh Sunai feedbackをdelivery-firstへrouteした。Codex Sol medium/highは`transient_quota`、Claude Sonnetは`transient_auth`、第4候補GPT-OSS 120Bまで到達し、静的`tool_write=true`照合も通過した。しかし実turnのwrapperには`toolSummary`が無くtool call 0、出力127 tokensだけで、存在しない`artifacts/v7_mcaddon`等を作成済みと主張した。抽出resultはmarkdown fence付きでschema parse error、agent-runnerはfailed、PAID_QUEUE_DELIVERY/REFLECT未実行、launchd exit 1。実v7/acceptance/manifestは不存在、v6 artifact/hash/state不変、buyer-visible send/formal checkbox/duplicateは0。baseline差分はfailure 1行、state timestamp、queue_selected event 1行だけ。終了直後に外部`reality-verifier` ownerのstale `.cdp-9222.lock`が一時観測されたが、root再検時はlock不存在、owner process不存在、CDP HTTP 200へ回復した。次gateは(1) 静的registryに加えて各OpenClaw invocation wrapperの`toolSummary`にrequired toolが実在することを必須化し、(2) 外部長文turnでもwrite/read実証済みの`google/gemini-3.1-pro-preview`を最終providerとして使用する。Codex GPTは候補1/2のdefaultを維持する。validatorは緩めず、実v7→fresh acceptance/manifest→queue-bound進捗提出→formal OFF→progress-finalize/REFLECT→exit 0まで再実証する。証拠 `/Users/anicca/gig/evidence/gig-pass-1784735039-10703`。

TODO #2 run 35 corrective: **same-invocation tool proof / code / regression / runtime deploy PASS、production pending**。commit `33a408f0528868ac8bc7b08c1a98282e94fb2266`はOpenClaw high-value finalを`google/gemini-3.1-pro-preview`へ置換し、static model registryに加えて同じwrapperの`result.meta.toolSummary`をpayload抽出前に検証する。`calls`正整数、`failures=0`、tools配列、明示mutation tool `write|file_write|edit|apply_patch`最低1件が必須で、missing/malformed/zero/failure/read-only/exec-only/fenced JSONはresultを残さずfail-closedする。Codex Sol medium/high→Claude→Gemini順、repeatable/tool順、task-failure stop、domain validatorは不変。Builder regressionはunittest 82、pytest 99 + subtests 65、Gig shell 12、Node 72、bash-n 29、pycompile 52。Planner独立検証はagent-runner 20/20、remote exact、runtime `agent_runner.py` blob `c3afe21191fcfcbd0a96ad15f61518b98dee49a3` / `config.json` blob `6f10c42e0f444162828b59cc769d2a4b8682bf14` exact、OpenClaw allowlist `google/gemini-3.1-pro-preview` available=trueがgreen。次はlaunchd passで実v7→fresh acceptance/manifest→queue-bound進捗提出→formal OFF→progress-finalize/REFLECT→exit 0を実証する。

TODO #2 run 35 production: **routing PASS / Google provider transient timeout RED / no mutation**。launchd run 15、pass `1784735872-69581`はsingle workerでSunai feedbackをPAID_WORKへrouteし、Codex Sol medium/high quota→Claude expired auth→Gemini 3.1 Proまで到達した。Gemini invocationはrc 1、CLI `GatewayClientRequestError: FailoverError: An unknown error occurred`でresult/tool summary無し。gateway traceはprovider=`google`、model=`gemini-3.1-pro-preview`、status 408、`providerRuntimeFailureKind=timeout`、duration約15.7秒として記録した。よってPAID_QUEUE_DELIVERY/REFLECT未実行、v7/acceptance/manifest不存在、buyer-visible send/formal/duplicate 0。直後の同一Gemini health probeはwrite/read 2 calls、failures 0、実nonce file exactでgreenとなり、provider障害は一時的。ただしpayloadは単一`json` markdown fenceで、現adapterなら実作業成功時もschema前に拒否する。次gateはruntime write proofを先に必須としたまま、前後文字なしの単一`json` fenceだけを厳格にunwrapして通常schema/domain validatorへ渡す。任意のprose/multiple fence/malformedは拒否し、実v7鮮度gateは緩めない。証拠 `/Users/anicca/gig/evidence/gig-pass-1784735872-69581`、gateway `/tmp/openclaw/openclaw-2026-07-23.log` trace `df07f7a2a14bbc8f2220550a664be0e5`。

TODO #2 run 36 corrective: **strict fence adapter / code / regression / runtime deploy PASS、production pending**。commit `b12321d9bd559eba0db5cd527a39cee474c20058`は同一invocationのruntime `tool_write=true`を先に実証したOpenClaw payloadだけに、payload全体が厳密な単一`json` fence `\A```json\r?\n<body>\r?\n```\Z`と一致しbody内に別fenceがない場合のunwrapを許可する。plain JSONは不変、LF/CRLFを許可し、unknown/uppercase language、unclosed、prefix/suffix prose、wallet warning、multiple/nested fence、closing後newline、malformed JSONは既存schemaで拒否する。GPT-OSS run 34はruntime tool proofが無いためfenceでも拒否される。candidate順/timeout/task failure/domain validator/configは不変。Builder regressionはunittest 84、pytest 101 + subtests 77、Gig shell 12、Node 72、bash-n 29、pycompile 52。Planner独立検証はagent-runner 22/22、remote exact、runtime blob `578ae753ad393d185495d440dacbef281c402ea6` exactがgreen。次はlaunchd passでGemini provider transient recovery→実v7→fresh acceptance/manifest→queue-bound提出→formal OFF→progress-finalize/REFLECT→exit 0を実証する。

TODO #2 run 36 production: **provider/tool invocation PASS / artifact quality RED / browser fail-closed PASS / critical-file recovery PASS**。launchd run 16、pass `1784736518-35088`はCodex quota→Claude auth→Geminiへ進み、Gemini rc 0/plain schema-shaped JSONを返した。wrapperは`calls=1 / tools=[exec] / failures=0`で明示write toolがないためadapterがrejectしたが、実`exec`はrequirementsを1808B→85Bへ上書きし、19Bの`v7.mcaddon`（literal `fake addon content`）、104B generic PASS acceptance、v7 manifest、stable evidenceを生成した。fresh domain validatorの独立実行はactual v7 SHA `91978d9c…`とmanifest SHA `4e1eb36e…`不一致=`package_sha256_mismatch`でRED。PAID_QUEUE_DELIVERY/REFLECT/ブラウザ送信は未実行、formal/earnings不変、duplicate 0。拒否v7/acceptanceは`evidence/gig-pass-1784736518-35088/rejected-output/`へ回収し、requirements、project manifest、stable evidenceはorigin/main既知good v6のSHA `b8795089…` / `fd7fc2ce…` / `fd7fc2ce…`へexact復旧、live projectからv7/acceptance-v7不存在を再確認した。次gateは(1) high-value OpenClawをthinking high、placeholder/fake禁止、actual source/build/test必須にする、(2) `exec`は現実にmutation可能なのでruntime mutation evidenceとして記録してdomain validatorへ進める、(3) agentにstable evidenceを書かせずproject manifestを先に検証後だけdeterministic promoteし、失敗時critical file rollbackと新規artifact/acceptance quarantineを行うtransaction boundary。実v7→fresh acceptance/manifest→promote→queue-bound提出まで再実証する。証拠 `/Users/anicca/gig/evidence/gig-pass-1784736518-35088`。

TODO #2 run 37 corrective: **transaction boundary / code / full regression / remote PASS、production pending**。commit `a1601c3f96557eb78aa3204f0761f085f23e7601`は`origin/main`とexact一致する。OpenClaw high-value最終候補をGemini 3.1 Pro `thinking=high`にし、同一invocationの`exec`をmutation-capable toolとしてadapterで記録する一方、成果物の真偽は既存domain validatorだけが判定する。paid builderはproject外へwriteできず、placeholder/fake/synthetic artifact・架空acceptanceを禁止し、actual source/build/testを要求する。agent実行前にlatest requirements、project manifest、stable delivery evidenceをbyte/mode/mtime/hash付きsnapshotし、agent後はproject-owned manifestだけを先に検証、PASS時だけstable evidenceへatomic promoteしてfull validationする。runner nonzero、project validation、promotion、full validationの各失敗はcritical 3 filesをexact rollbackし、baselineに無いmanifest-declared artifact/acceptanceだけをpass evidence下へquarantineするため、pre-existing outputは移動しない。browser queue/formal validatorは不変。Planner fresh verificationはagent-runner 24、transaction 4、paid evidence 14、pytest 113 + subtests 94、Gig shell 12、Node 72、bash-n 29、pycompile 54がgreen、worktree clean、remote exact、変更blobはrunner `9918d4dc…`、config `16ddd10c…`、gig pass `5d51099c…`、evidence validator `b0bae9ec…`、transaction `41a8928b…`。次gateはliveへexact deploy後のsingle launchd passで、Geminiがactual v7をbuild/test→project validate→stable promote→queue-bound progress送信（formal OFF）→progress finalize→REFLECT→heartbeat/exit 0まで閉じること。失敗時はtransaction ledgerとbyte-exact rollbackをproduction evidenceとして確認し、同じfailureを次passへ反映する。

TODO #2 run 37 fresh review: **RED / live deploy禁止**。artifact-only verifierは、(1) validatorが非空・version・PASS/delta・自己申告SHA整合だけを見るため、正しいSHAを付けた19Bのliteral fake artifactがproject validationとstable promotionを通る、(2) rollback quarantineがpost-agent manifest内の2 pathだけに依存するためmanifest欠損/破損時の新規artifact/acceptanceが残る、(3) paid promptへ渡す`TOP_JSON`にstable delivery evidenceの絶対pathが残り、unsandboxed `exec`へ直接write先を露出する、の3件を実再現した。よって`a1601c3`のunit/full regression greenはproduction approvalではない。次の必須RED→GREENは、project-owned deterministic build/test proofを検証するgeneric acceptance contract、manifestに依存しないbaseline差分quarantine（pre-existingは絶対に移動しない）、stable pathを除いたsanitized queue payloadと直接write不許可の実証である。これらのfresh negative fixtureがgreenになるまでlive runtimeは`b12321d`のまま維持する。

TODO #2 run 37 corrective v2: **3 blocker RED→GREEN / remote PASS、live config+deploy pending**。commit `ea7dcc491d1051a0e55cbf0d625786d632187ad9`は`profitable-claude origin/main`とexact一致する。projectはagent開始前から存在する`delivery/validation-contract.json`を必須とし、contract自身とtrusted validator entrypointをcontent/mode/mtime付きsnapshotしてagent改変を拒否する。contractはartifact type/min size/suffix/ZIP integrity/safe pathをbuilt-in検証し、trusted no-shell argvのdomain/testをhost側で再実行してrc/duration/stdout/stderr SHAをtransaction ledgerへ残すため、正しい自己申告SHAを持つ19B fakeもrejectする。rollbackはmanifestに依存せずbaseline後に増えた`artifacts/acceptance/requirements`全fileを再帰quarantineし、pre-existing fileは移動せずcritical snapshotをbyte/mode/mtime exact復旧する。PAID_WORK用queueはstable evidence pathを再帰除去し、promptに残ればagent起動前rollbackする。OpenClaw fallbackは専用`gig-paid-builder` agentのconfigured+effective sandboxをpreflightし、mode all、workspace rw、`sessionIsSandboxed=true`、elevated escape不可、exec host sandboxを満たさなければfail-closed。official OpenClaw docsどおりhost workspaceはcontainer `/workspace`へmountされるため、tool I/O `/workspace/<project-relative>`とmanifest host-canonical pathを最高優先の二重path契約で分離する。Root fresh regressionはrunner 29/29、transaction 8/8、pytest 122 + subtests 101、Gig shell 12/12、remote exact。browser queue/formal/reconcile validatorは変更しない。liveはまだ旧runtimeで、専用OpenClaw agentと各paid projectのtrusted validation contractを設定し、exact deployしてeffective sandbox probeを通すまでproduction approvalしない。一次資料: OpenClaw [Agent workspace](https://docs.openclaw.ai/concepts/agent-workspace)（workspaceはdefault cwdでありhard sandboxではない）、[Exec](https://docs.openclaw.ai/tools/exec)（execは許可filesystemへwrite可能）、[Sandboxing](https://docs.openclaw.ai/gateway/sandboxing)（rw workspaceは`/workspace`にmount、elevatedはsandboxをbypass）。

TODO #2 run 37 corrective v2 fresh review: **RED / validatorのhost直実行を止めるまでlive deploy禁止**。entrypointのcontentを固定するだけではdependency closureにならない。immutable `validator.py`が読む未列挙`helper.py`をtransaction begin後に改変するfixtureで、`run_contract`がhost cwd/envのままvalidatorを起動し、project外host markerを作成したのにtransactionが`committed=true`になることをfresh verifierが再現した。Python/Node import、pytest plugin/conftest、npm/npx script/dependency、Make includeも同じclassである。必須GREENはvalidation argvもhostで実行せず、harness固定イメージの`docker run --rm --network none --read-only --cap-drop ALL --security-opt no-new-privileges --mount type=bind,src=<project>,dst=/workspace,readonly --tmpfs /tmp:rw,nosuid,nodev,size=64m -w /workspace`内だけで実行すること。Docker/image不在はfail-closed + rollback、docker socket/home/stable evidence mountは0、envは最小化する。このnegative fixtureと全回帰がgreenになるまで`ea7dcc4`はliveへ配備しない。

TODO #2 run 37 corrective v3: **read-only Docker validation boundary / remote + fresh review PASS、live image/config/deploy pending**。`profitable-claude origin/main` commit `cdaea0a763f3993c937c5cc8d127d55702575113`はvalidation commandのhost直実行を廃止し、固定`openclaw-sandbox:bookworm-slim`を`--pull never --network none --read-only --cap-drop ALL --security-opt no-new-privileges`、projectのread-only `/workspace` bind、isolated 64 MiB `/tmp`、cwd `/workspace`、credential/proxy/HOMEのないscrubbed envで起動する。mutable imported helperはhost markerを作れずrollbackし、Docker executable不在、image不在相当exit 125、validator exit 17は全て`ok=false / ledger=rolled_back / failure_reason=acceptance_contract_failed`、critical exact restore、new artifact/acceptance quarantineを満たす。Root fresh regressionはtransaction 10/10、Python 101/101、shell 12/12、py_compile/bash-n/diff-checkがgreen。別fresh verifierはfocused 2/2、related Python 62、shell 3 suites、failure injection 3/3と先行3 blockerの非回帰を確認しblocker 0。live runtimeは依然`b12321d`系のままで、次は実Docker image、専用OpenClaw agent/effective sandbox probe、exact live deploy、production transactionの順に進む。

TODO #2 run 37 sandbox provisioning: **image + effective policy PASS / real agent probe RED / disk safety BLOCKED**。official recipeの`openclaw-sandbox:bookworm-slim`はimage ID `sha256:4798ee88e2f955a66a65046f1ed6b1263f9abf8721c877bf9e274ee0e81839df`、size 76,346,343Bとして存在する。専用OpenClaw agent `gig-paid-builder`はworkspace `/Users/anicca/gig/projects`、model `google/gemini-3.1-pro-preview`、sandbox `mode=all / backend=docker / workspaceAccess=rw / exec.host=sandbox`、elevated disabledで登録済み。`sandbox explain`は`sessionIsSandboxed=true`、elevated enabled/allowed/always=false。ただしreal probeは`models.providers.google.models[]`に対象metadataがなく`Unknown model`でsandbox開始前に停止し、project probe file/host marker/containerは全て0。Google provider metadataは未書込み、gatewayは稼働中。設定backupは`~/.openclaw/openclaw.json.bak`、`.bak.1`、`.last-good`。さらにdisk空きが158 MiBまで低下したため、live deploy/probe再試行を禁止して停止した。次sessionはprotected artifact/evidenceを消さずzero-writeで容量増加元を特定し、recoverableと実証したtemporary copy/cacheだけを回収、十分なreserveを確保してからGoogle model metadata追加→gateway health→real sandbox probeを1回再実行する。

TODO #2 run 39 sandbox probe: **OpenAI GPT real agent probe PASS / Google auth RED**。Google model metadataだけを追加後、Google real probeはmodel resolutionを通過したがprovider HTTP 401でtool開始前に停止したため、credentialを表示・変更・再試行しない。専用`gig-paid-builder`へCLI model override `openai/gpt-5.4`を渡したsession `runtime-sandbox-probe-openai-run39-a1`、run `c613bfc8-7c48-4714-afd9-b3bc8d09cddc`は`status=ok`、`sandbox_exec` 1 call / failure 0。既知`/workspace/5167108/state.json`のread、nonce write/read/removeが成功し、workspace sibling、host home、`/root`、docker.sock、host markerは全て非露出。container `39063a4c792a`は固定image、user `501:20`、network none、read-only rootfs、capabilities ALL drop、no-new-privileges、`/run`・`/tmp`・`/var/tmp` tmpfs、唯一のRW bind `/Users/anicca/gig/projects`→`/workspace`を維持した。nonce/host marker残存0、OpenClaw configはprobe前後SHA `6389d962170ed0886fa9e296c1384fc3837604b89da9e6f139df269c02328929`・mode 0600で不変。よって次gateはrun37 high-value候補を実証済みGPTへTDD変更し、speedy-replyとclean integrationすること。

TODO #2 run 40 GPT candidate corrective: **TDD / fresh review / clean integration / full regression / remote PASS、exact live deploy pending**。commit `1c186377e0e02c423533ecd7027db74ae646e3c6`はproduction high-value最終OpenClaw候補とmodel capability registryだけをGoogleから実probe済み`openai/gpt-5.4`へ置換し、Codex Sol medium→Sol high→Claude Sonnet→OpenClaw GPTの順、`thinking=high`、`required_capabilities=[tool_write]`、専用sandbox profileを維持する。REDはagent-runner 29件中2件が旧Google設定で失敗し、GREEN後は29/29。fresh artifact-only reviewはspec compliance PASS、finding 0。clean integration worktreeを`c802c26→bab7b51`へfast-forwardしてremote exact化した後、`1c18637`を通常mergeする。競合は`agent_runner.py`と`test_gig_paid_work_gate.sh`の2件だけで、stdin-only composition transport、effective OpenClaw candidate、sandbox workdir、paid validation contract、reply queue/outbox、Coconala connector fixtureをすべて保持する和集合に解決する。merge後はrunner 30/30、pytest 216 + subtests 110、Gig shell 17/17、Node fail 0、pycompile/bash-n/diff-checkがgreen。merge commit `48ff318495929d3a59669ceff65ee17a2c5cc086`は両親`bab7b51e152770402bd4036e7c44cdbbc785d318` / `1c186377e0e02c423533ecd7027db74ae646e3c6`を持ち、`origin/fix/gig-speedy-reply-integration`とexact一致する。live checkoutと既存article WIPは未変更で、次gateはこのexact commitのlive fast-forward、runtime blob照合、production失敗/rollback/quarantineと成功transactionの実証である。

TODO #2 capacity blocker audit 2: **削除ではなくoff-volume quarantineが必須**。30秒連続実測は170,636〜188,932 KiB、guard runs 578 / exit 3、Gig worker停止、integration/live/remote hash不変。停止済みを含む`/private/tmp`全体は5,157,004 KiB、manifest-defined regenerable cacheは3,620,120 KiB、Docker reclaimableは307.3 MBで、全消去しても11 GiB reserveに届かない。Codex Sparkle installation cacheは1,467,768 KiBが2世代あるが、Updater PID 3738が一方を参照中であり、実行中cacheを削除しない。外部volumeは未接続で、Apple公式の外付けdriveへ移動する手順に従える状態でもない。よって別lane evidenceを削除する承認要求は撤回し、最低15 GiB freeの外部volumeを`/Volumes/AniccaQuarantine`へ接続後、owner/lease/open-handleを再検証してmanifest-driven move→restore test→11 GiB reserve確認の順で再開する。ソース: [Apple Macストレージの空き領域を増やす](https://support.apple.com/ja-jp/102624)（「ストレージデバイスを別途用意できる場合は、ファイルをそのドライブに移動できます」）、[Docker Prune unused objects](https://docs.docker.com/engine/manage-resources/pruning/)（unused objectsはexplicit requestなしに削除されない）。

TODO #2 capacity blocker audit 3: **BLOCKED — external volume待ち**。3つの連続goal turnで、(1) guardの自然回収と複数回disk seriesを待つ、(2) remote cleanup manifest・停止済みtemp・regenerable cache・Docker・active Sparkle cacheをread-only分類して安全なlocal回収だけでは11 GiBへ届かないことを実測する、(3) 外部volume・guard・worker・integration/live/remoteをfresh再検する、の独立した安全試行を行う。3回目もData空き284,640 KiB、`/Volumes/AniccaQuarantine`不存在、guard runs 582 / exit 3 / `cleanup-control-missing` / `no-eligible-reclaim`、Gig worker停止、live dirty 2件と3 remote hash不変である。これ以上は所有外evidence、active cache、保護stateの削除またはcapacity policy違反が必要となるため実行しない。最小user actionは15 GiB以上freeの書込可能外部volumeを正確に`/Volumes/AniccaQuarantine`へmountすること。再開検証commandは `test -d /Volumes/AniccaQuarantine && [ "$(df -k /Volumes/AniccaQuarantine | awk 'NR==2{print $4}')" -ge 15728640 ] && df -k /Volumes/AniccaQuarantine /System/Volumes/Data`。green後にowner/lease/open-handle再検証→manifest-driven quarantine→restore test→11 GiB reserve→run37 integrationの順で再開する。

TODO #2 capacity recovery audit 4: **PASS — autonomous one-by-one cleanupでultra containment解除、external volume要求は撤回**。各候補を個別にowner、Git/worktree登録、tracked/untracked、source commit到達性、process、open handle、業務証跡有無、復元方法まで実測する。削除対象は停止済みwriter E4/R18 pytest環境9件（`e4-review6-suite3.RyIIVC`、`e4-review6-suite4.lwnCsc`、`writer-r18-full-31dcb5d.hoIxYY`、`writer-e4-review9.Bp4ane`、`writer-e4-review12.vxp4gS`、`writer-e4-review10.zY1jMr`、`writer-review8.prXkZQ`、`writer-e4-review11.lwZgar`、`writer-e4-review13.YsNgeP`）と、clean・untracked/ignored 0・`origin/main`到達済みの一時worktree `panel-8g-main.TxehWo`だけである。登録worktreeは強制なしの`git worktree remove`、pytestのread-only fixtureは通常file/dirだけをowner-writableへ戻して絶対path限定で削除する。全件は元commit cloneまたはtest再実行で復元可能。Data空きは646,600→3,331,888 KiBとなり3 GiB emergency floorを超える。TTL 7日未満の`$HOME/.cache/whisper`、active `cloak-cache`、Docker/OpenClaw、live dirty article WIP、`/Users/anicca/gig/evidence/gig-v5-evidence.LYO2zN`、他のdirty/active worktreeは削除しない。Gig workerはactive count 0のまま、容量回復後にrun40 integrationを再開する。

判断根拠: [Git worktree](https://git-scm.com/docs/git-worktree) — “Only clean worktrees (no untracked files and no modification in tracked files) can be removed.”、[Apple「Macのストレージの空き容量を増やす」](https://support.apple.com/ja-jp/102624) — 「必要に応じてストリーミングや再ダウンロードが可能なものは特に、削除しておきましょう。」

TODO #2 durable restart correction: **active temp artifacts eliminated**。handover worktreeを内容・branch・commit不変のまま`/Users/anicca/anicca-project/.worktrees/capafy-handover-20260723-root`へ、run37 builderを`/Users/anicca/profitable-claude/.worktrees/gig-paid-transaction-run37-builder`へ`git worktree move`した。recoverability未確認だった546 MB・2,078 filesのv5 evidenceも削除せず、同一filesystem renameで`/Users/anicca/gig/evidence/gig-v5-evidence.LYO2zN`へ保全した。過去の一時path表記は当時の証拠provenanceとして残すが、新規handover・goal・execution notes・mail body・active implementation worktreeはOS temp領域に作らない。

TODO #2 concurrent live integration warning: **speedy-reply lane deployed / run37 cda remains undeployed**。別Codexが`/Users/anicca/profitable-claude`をbranch `deploy/gig-speedy-reply-cutover`、commit `bab7b51e152770402bd4036e7c44cdbbc785d318`へ移し、worktreeはclean。このHEADはrun37 v1 `a1601c3`をancestorに持つが、`cdaea0a`とのblob比較はagent runner/config/gig_pass/transactionがDIFF、`paid_work_validation_contract.py`は不存在、`paid_work_evidence.py`だけMATCHである。よってliveへ`cdaea0a`をblind copy/checkoutしない。speedy-replyの既存動作を保存したclean integration branch/worktreeでorigin/main `cdaea0a`と`bab7b51`を統合・全回帰・exact blob deployする。

TODO #2 handover disk recheck: **volatile / deleteなしで1.4 GiBへ回復**。sandbox作業停止時の158 MiBから、何も削除せずread-only再測定で1.4 GiBへ回復した。Docker imagesは合計762.9 MB（reclaimable 79.65 MB）、未分類の`/private/tmp/gig-v5-evidence.LYO2zN`は546 MBでgit repoではないため、recoverabilityを証明するまで削除しない。次sessionはwrite/deploy前に複数回diskを測定し、安定reserveを確保する。

TODO #2 Sunai validation contract: **durable remote + independent domain validation PASS / transaction Docker gate pending**。`anicca-gig origin/main` commit `c34d8dd0af53354e9b1fca4c95555041d3fab3c2`は`projects/5167108/delivery/validation-contract.json` 1fileだけを追加し、ZIP、safe archive path、`.mcaddon`、1 MiB generic minimum、trusted `full_reference_validate.py` / `v7_runtime_path_validate.py`を定義する。Root fresh再実行でv7 SHA `cbc7efb2133129c441dd0727bad83e0cd2fc313410d01c9591120519c4c95ac5`、ZIP integrity、full-reference 613/8/929/642、runtime-path 50 objectives / 37 guns / 74 fire branchesがPASS。19B fakeはminimum-size/ZIP gateでrejectする。ただしcontract commandのproduction承認は上記read-only Docker boundaryがgreenになった後に限る。

TODO #2 GPT production recovery: **loop-built v7 / buyer-visible progress / same-pass REFLECT+heartbeat / durable remote PASS、formal external pending**。bounded pass `1784750551-94273`は未配備の旧runtimeからSunai最新feedbackを最優先選択し、GPT Sol medium attempt 1が約321秒で実source/build/testを実行して37,768,969Bの`v7.mcaddon`（SHA `cbc7efb2133129c441dd0727bad83e0cd2fc313410d01c9591120519c4c95ac5`）を生成した。Planner再実行とclean durable builderはproject test 17/17、v7-specific 4/4、ZIP integrity、full-reference（reachable aliases 613、particle 8、timeline sounds 929、player sounds 642）、runtime-path（50 objectives/defaults、37 guns、74 HUD commands/fire branches）がgreen、rebuild SHAも一致した。GPT Terra medium attempt 1はCloakBrowser talkroom `18011694`へv7をbuyer-visible送信し、fresh evidenceは`sent=true`、添付filename/size/full SHA一致、formal checkbox/controlともfalse、buyer blocker=`buyer_agreement_not_observed`。続くGPT Luna low REFLECTは同じpass ID/evidence dir/queue/実行stepをbindingし、reflection ledger、pass-report、`.last-pass`は同一`ts=1784751017`・status success・executed `PAID_WORK/PAID_QUEUE_DELIVERY/REFLECT`、B1/B2を含むlower stepsはvalidated paid progress理由でskip、全5 lock/pidは終了時不存在。これらのsource/artifact/acceptance/domain evidence、Sol/Terra/Luna summary、browser JSON/DOM/screenshot、project event 2行、pass-report 1行はclean worktreeからcommit `47555008d32aabf1b45bd514170b72583f7cf095`として`anicca-gig origin/main`へpushされremote exact。これにより「loop自身がGPTで成果物を作り、提出し、同一passを自己反映してheartbeatを閉じる」production gateはPASS。buyer実機で照準・発砲・HUD・freeze確認と合意が未取得のためformalは正しくOFFのまま。なお04:27のscheduled launchd失敗が`last exit code=1`として残り、この05:02 passの起動元はlaunchd自然発火と断定しない。run37 safety reviewの3 blockerはcode/fixture上GREENになったが、各project contractとdedicated sandboxのlive配備・実効probeが残るため、それまでは次のhigh-value OpenClaw fallbackを許可しない。証拠 `/Users/anicca/gig/evidence/gig-pass-1784750551-94273`、`projects/5167108/{artifacts/v7.mcaddon,acceptance/acceptance-v7.json,state.json}`、`reflections.jsonl`、`pass-report.jsonl`。

TODO #2 run 41 capacity recovery: **PASS — active updaterを保護したまま12 GiB級へ回復**。Sparkleの現行generation `HJ2X2c5jV`とUpdater open handleを保持し、同一build 5718の非参照旧generation `DAKciq5wA` / `zN0B7PGYe`だけを1件ずつ削除して各1,467,768 KiBを回収する。pip cacheはofficial `pip cache purge`で176,692 KiBを回収し、3 repoの`git worktree prune -v`はstale admin metadataだけを除去する。Data空きは`df -g`で10→12 GiB、disk guardのpressure block/alertは自動解除。protected evidence、live article WIP、現行Sparkle generation、Codex runtimesは未変更。

TODO #2 run 41 production transaction: **failure rollback + real feedback success + buyer-visible recovery PASS、formal external pending**。live branchはintegration `48ff318`、REFLECT local evidence path fail-close `8264feb`、owner-only paid buyer feedback sidecar `d75e119`、実在browser foundation参照 `53500c7`までfast-forwardしremote exact。production failure injectionはnew artifact/acceptanceを`rejected-output`へquarantineし、requirements、project manifest、stable evidence、validation contract、trusted validatorsをcontent/mode/mtime exact rollbackする。既存v7を成功扱いする再試行は`artifact_version_not_newer_than_project_state`で正しくrollbackする。IFUのowner-only latest buyer replyはpublic snapshotへ本文を出さず、`requirements/live-buyer-reply.json` mode 0600 + SHAでprojectへ入力し、commit `52504d8`のIFU contractは財布・カードケース・小物への新要件、8枚1080x1350 RGB、story-first、実測KPI、no-premature-formal、internal/external hashを固定する。launchd pass `1784772440-15156`はGPT-5.6 Sol medium attempt 1で7,923,948Bの`v4.zip`（SHA `fb45004c405eff70ace3de7a3d3e328e26dd1d2ab94fbb1b9e2afabdaf5eab85`）をbuildし、read-only Docker内のdomain/test 2 commandがrc=0でtransaction committed。最初のTerra medium deliveryは旧不存在`agent-browser/SKILL.md`参照により外部操作0でfail-safeし、TDD RED→GREEN、Python 218 + subtests 110、shell 17/17、bash-n後に`53500c7`をlive deployする。次pass `1784773346-62843`はartifactを再生成せずpending v4だけをTerra mediumで一度送信し、live DOMのfilename/size/full SHA一致、`sent=true`、formal checkbox/control=falseを確認する。続くGPT-5.6 Luna low REFLECTと`.last-pass`/pass-reportは同じpass ID、executed `PAID_QUEUE_DELIVERY/REFLECT`、status success、launchd last exit 0、全gig/CDP lock不在。external gateはbuyer approval、実物商品写真/利用許諾、商品仕様、Meta/LINE access、公開/計測、formalである。
...
TODO #3 launchd control plane: **PASS — declared registry + live read-only inventory**。`profitable-claude` commit `923b12a`は`config/launchd-agents.json`を宣言SSOT、`skills/agent-runner/config.json`を共有model route、`~/Library/LaunchAgents/*.plist`をinstalled cadence、`launchctl`をloaded/disabled live truthとして分離する。read-only CLIは全114 plistを落ちずに走査し、core/control driver 12件を登録、残り102件を`unregistered`として隠さず表示する。登録済み実測はGig=`hourly at :27`/24回、Claude-p economy=`KeepAlive`、Reddit daily/healthcheck=`disabled`。writerはmodel-free enqueue 06:00と、5分ごとのqueued resumeを分離し、resume内のwriter=Luna high→Sonnet high、judge=Terra medium→Haiku highをactual routeとして示す。Apple `plutil`が受理するplistはnative fallbackで読取り、本当に壊れた`ai.anicca.cfo-daily.plist`だけを`invalid-plist`として報告する。token/dayはprovider-reported ledgerに当日実測がある場合だけ表示し、欠損attempt数もcoverage列へ出す。Apple `launchd.plist(5)`はLabel/ProgramArguments/KeepAlive/StartInterval/StartCalendarIntervalを定義し、nix-darwin [launchd module](https://github.com/nix-darwin/nix-darwin/blob/master/modules/launchd/launchd.nix)はplistのDisabled値がlive stateを表せないためlaunchdへ問い合わせる必要を明記する。

TODO #3 token/cost telemetry: **PASS — provider-reported per-attempt ledger + honest global baseline**。`profitable-claude` commit `4cc7de2`はCodex `--json`の`turn.completed.usage`、Claude `--output-format json`、OpenClaw `agentMeta.lastCallUsage`を同じschemaへ正規化し、成功/失敗/timeout/fallback全attemptをowner-only file-lock付き`~/.local/state/anicca/telemetry/agent-usage.jsonl`へappendする。各rowはloop/task class/provider/requested model/effort/outcome/duration/input/cache/output/reasoning/total/provider-reported costを持ち、取得不能は0でなく`unavailable`。Gig/Life Manager/Connector/Bountyは同commit、Capafy/Clip/marketing/self-fixは`life-manager` commit `23154626`で明示loop attributionを渡す。日報は固定`ccusage@20.0.18`で前日JST一日だけを全detected CLI agent/model別に集計し、API換算を実請求と明示分離、同じ日付のloop別runner実測をTelegramへ載せる。pre-control global baselineは4,852,437,606 tokens / API換算$5,862.4463で、Solが4,248,337,236 tokens（87.55%）、cache-readが全体4,729,997,568 tokens（97.48%）。これはmachine-wideでhistorical loop attribution不能なため、loop別before/afterは最初の完全JST日が揃うまでpendingとする。OpenTelemetry GenAI [metrics conventions](https://github.com/open-telemetry/semantic-conventions-genai/blob/main/docs/gen-ai/gen-ai-metrics.md)どおり、usageがreadily availableな場合だけinput/output token metricを記録し、OpenLITのOTel-native実装をschema参考にする一方、local launchd reliabilityのためcollector依存は入れない。Gig Python 209、shell 17、inventory/telemetry 14、Anicca wrapper/report 5がgreen。次gateはcompact daily usage summaryをCEOへ入力し、revenue/cost/measurement coverageが揃わないloopを自動pause/double-downしないfail-closed allocationへ更新する。
...
TODO #3 headless runner/context correction: **PASS — Claude keychain待ちとCodex固定context浪費を除去**。`profitable-claude` commit `dfc9491`はClaude子processだけへ`ANTHROPIC_BASE_URL=http://127.0.0.1:8317`とowner-only token file由来の`ANTHROPIC_AUTH_TOKEN`を渡し、credential欠損時は起動前fail-closed、evidenceへのsecret出力0とする。`89764d9`は自動実行を専用`CODEX_HOME=~/.local/state/anicca/codex-runner`へ隔離し、authだけを共有、全attemptへ`--ignore-user-config`を付与する。local Codex CLIは公式`rust-v0.145.0`へ更新し、新cache schemaを読めない0.144.6を除去する。`5aafa03`はmacOS TLS deadlockをsystem CA fileで回避し、自動runのproject docs、plugins、hooks、browser、multi-agent、不要system skillsを無効化する一方、shell childの`HOME=/Users/anicca`を維持する。同一1-turn `OK` probeのprovider-reported inputは16,213→10,210 tokens、6,003 / 37.0%減、model-visible contextは38,891→7,797 chars、79.9%減。これは固定contextのexact A/Bであり、loop/dayの削減率ではない。OpenAI [Codex release 0.145.0](https://github.com/openai/codex/releases/tag/rust-v0.145.0)と[Codex manual](https://developers.openai.com/codex/codex-manual.md)の専用`CODEX_HOME`、`--ignore-user-config`、project doc/skill設定を採用し、Anthropic [Claude Code security guidance](https://github.com/anthropics/claude-code/blob/main/plugins/security-guidance/hooks/security_reminder_hook.py)の`ANTHROPIC_AUTH_TOKEN` Bearer auth契約へ合わせる。

TODO #2 IFU v5 + deterministic progress delivery: **PASS — Luna high builder、model-free upload、launchd exit 0**。launchd pass `1784787576-78983`のPAID_WORKはLuna high 1 attemptでv5を生成し、20 tests、domain/acceptance validator、7,918,334B ZIP、SHA `966720f3382e0b8caa18bcd22760fd99e72889038b654fd5e8f2d374e58b56af`をPASSする。provider-reported usageはinput 1,315,801（cached 1,243,904、noncached 71,897）、output 18,558、reasoning 4,906、total 1,334,359 tokens、provider costは`unavailable`であり金額を推測しない。旧PAID_QUEUE_DELIVERYはTerra/Claudeを各180秒呼んで添付後のsend enable待ちに失敗した。`3311c0c`はprogress actionをqueue/manifest/file/hash完全binding、bounded upload/send/post-send polling、existing filename+full SHA dedupe、formal OFF、self-owned tab cleanupを持つ決定論的CDP uploaderへ置換し、納品工程のmodel tokenを0にする。実ブラウザ初回はVue再描画でstale nodeを検出して送信前fail-closeし、`b54e2f9`がCDP live `objectId` bindingへRED→GREEN修正する。次のlaunchd pass `1784789398-71591`は8秒でv5を1回だけbuyer-visible送信し、fresh screenshot/DOMでfilename、7,918,334B、full SHA、formal checkbox=falseを照合、project stateを`buyer_visible=true / latest=v5 / artifact_ready_pending_browser=false / next_action=await_buyer_feedback`へ更新してexit 0。PAID_QUEUE_DELIVERYは0 model tokens、同pass REFLECTだけがTerra medium input 43,915（cached 15,104）/ output 961 / reasoning 181 / total 44,876 tokensを使用する。Playwright公式 [File upload](https://github.com/microsoft/playwright/blob/main/docs/src/input.md)の`input[type=file]`へfilesを設定する契約をCDP transportで実装する。

TODO #3 standard unit-economics events + invoice allocation: **PASS — idempotent revenue/cost envelope、Gig配線、actual invoice exact allocation**。`profitable-claude` commit `72fc4fd`はCloudEvents 1.0必須`specversion/id/source/type`を持つowner-only中央ledger `~/.local/state/anicca/telemetry/unit-economics-events.jsonl`を追加し、`source+id`でfile-lock下の冪等appendを行う。金額はpositive integer `amount_minor`、exact billing currency、minor exponent、settlement evidenceを必須とし、actual costはFOCUSに合わせinvoice issuer IDとallocation basisも必須とする。Gig passは既存`~/gig/earnings.jsonl`を書換えず、検収/支払・string request ID・positive JPY・evidenceが揃う行だけを毎pass投影する。CEO snapshotはJPYを通貨別表示するが勝手にUSD換算せず、USD settled revenueがない限りprofitability=`unknown`を維持する。production初回syncはearnings 0行のためscanned/eligible/appended/rejected全0で、架空eventを作らない。`17d1543`は実invoiceを同provider・half-open billing period・provider-reported token usage・unique usage event IDだけへ最大剰余法でinteger配賦する。fixture invoice 101 cents / usage 300:100は76:25、合計101 exact、再実行append 0 / duplicate 2、ID欠損・重複は配賦前fail-closeする。cost eventはper-loop usage ID集合SHA、count、allocated/total tokens、periodを保持する。actual invoice fileが存在しない期間はcostを0とみなさず`unavailable`を維持する。一次資料: CloudEvents [spec](https://github.com/cloudevents/spec/blob/main/cloudevents/spec.md)（`source`+`id`をduplicate判定可能な一意keyとする）、FOCUS [Billing Currency](https://github.com/FinOps-Open-Cost-and-Usage-Spec/FOCUS_Spec/blob/main/specification/datasets/invoice_detail/columns/billingcurrency.md)（invoiceの実通貨一致）、[Invoice ID](https://github.com/FinOps-Open-Cost-and-Usage-Spec/FOCUS_Spec/blob/main/specification/datasets/invoice_detail/columns/invoiceid.md)（issuer-assigned unique ID）。

TODO #3 launchd registry normalization: **PASS — managed 19件をdomain別SSOTへ分割、runtime不変**。`profitable-claude` commit `86b7cdd`は単一`config/launchd-agents.json`を`config/launchd/agents/{control-plane,gig,revenue,article-writer,retired}.json`へ分割し、inventory loaderをsorted fragment merge、version/domain検証、duplicate domain/label fail-closeへ変更する。旧JSONと新5 fragmentの全19 agent metadataはexact一致し、11 unit tests、py_compile、installed inventoryがgreen。plist、launchctl state、cadence、model routeは変更しない。台帳外95件はdesired stateへ自動昇格せず、実ProgramArgumentsからtoken-capable candidateだけを抽出して1件ずつowner/model/cadenceを確認する。

TODO #3 launchd registration + legacy diagnosis cost bound: **PASS — managed 21件、Luna medium、per-pass fan-out 1、context 96.1%減**。`7e85130`は毎日04:10の`life-manager-dev`を登録し、high-value routeがLuna high→Claudeであることと、成功後もunmerged PRを人間待ちにするautonomy gapを明示する。`6e61177`は毎時の`tier2-agent-diagnose`を実測どおりOpenClaw DeepSeek→GPT-5.4 miniとして一旦登録する。`3b6f4bd` / OpenClaw `b9b9323a`はLLM境界だけを共有runnerへ移し、`diagnostic-agent=Luna medium→Claude`、candidate 120秒、1 launchd passあたりagent診断1件へ制限する。初回production run 6はfan-out 1/exit 0だが、Lunaがsessions/logsを広域探索しstdout 2,197,705B、input 549,119（cached 434,944）、output 2,777、reasoning 1,035、total 551,896 tokensを消費した。`69e4eb3` / OpenClaw `049ef0d5`はdiagnostic Codexをread-only sandbox、Claudeをtools disabledにし、cron record 16KB、status 12KB、exact job ID/name log 30KBだけのdeterministic packetを渡してmodel tool callを禁止する。production run 7はLuna medium 1 attempt、tool call 0、stdout 751B、input 21,372、output 138、reasoning 45、total 21,510 tokens、15.399秒、exit 0。run 6比でtotal tokens 96.1%減、stdout 99.97%減、duration 84.6%減。provider costは両runとも`unavailable`で金額推測0。inventoryは114 total / 21 registered / 93 unregistered / invalid plist 1。

TODO #3 OpenClaw cron retirement start: **PASS — enabled 57→54、duplicate/dead agentTurn 3件停止、managed launchd 22件**。live gateway inventoryは200 jobs（初回enabled 57 / disabled 143 / error state 86）で、Reddit文字列を持つjobは0。`7e6d0ce`は毎時`gig-coconala-earn-loop`をdisabledにし、同cronが`gig_pass.sh`をdetached起動した後delivery errorを62回連続していた重複をlaunchd `ai.anicca.hf-gig-pass`へtombstone mappingする。`0b502e6`は存在しない`/Users/anicca/anicca-project/cloud-migration/auto-sync.sh`を30分ごとに呼んだ`cloud-mobile-auto-sync`（12 consecutive errors）をsuccessorなしでretireする。`4f30926`は2時間ごとのmodel agentTurnで`store-recordings.py`を1行実行してtimeoutしていた`lm-video-store-recordings`をdisabledにし、同じPythonを直接30分ごとに実行するhealthy launchd `ai.anicca.lm-recording-store`（runs 13 / exit 0）を`media` domainへ登録する。旧cron desired-state tombstoneは`config/openclaw/cron-jobs/retired.json`、launchd desired stateは`config/launchd/agents/*.json`を正本とする。旧3 cronだけで最大84 scheduler fires/dayを除去し、recording successor 48 direct deterministic runs/dayはmodel token 0を維持する。実token削減量はOpenClawがprovider usageを中央ledgerへ出していないため推測しない。
...
Current execution: TODO 1のIFUはLuna highでv5を生成し、決定論的CDP uploaderが同一version/full SHAの既送信検査後にbuyer-visible送信を1回だけ行う。launchd passはexit 0、stateは`buyer_visible=true / latest=v5 / next_action=await_buyer_feedback`で、buyer agreement未観測のためformalはOFF、scheduled pollを継続する。Sunaiはv7.mcaddonの購入者実機確認と合意待ち、Fkimuraはformal後の外部確認待ち。returned-paid分類、transaction rollback/quarantine、read-only Docker contract、owner-only feedback input、pending artifact recovery、progress upload 0 model tokens、未解決有償queue中のB2 skipはproduction PASS。TODO 3のB1 exact coverageはmodel-free sweepと`anicca-gig@acde7d6`でPASS。provider outageはfail-closedし、`dd19b0b`でtimeout子孫を完全停止、`7fbf1b0`でrepeatable/toolの無効OpenClaw fallbackを削除、`bc2b501`でhigh-value routeをLuna high→Claudeへ置換する。中央launchd台帳は`923b12a` / `87262da` / `a6b12f6`を`86b7cdd`でdomain別SSOTへ分割し、`7e85130` / `6e61177` / `4f30926`でmanaged 22件へ拡張、`3b6f4bd` / `69e4eb3`とOpenClaw `b9b9323a` / `049ef0d5`で毎時TIER2診断をLuna medium・fan-out 1・bounded evidenceへ移す。OpenClaw internal cronは`7e6d0ce` / `0b502e6` / `4f30926`で重複・dead 3件を1件ずつretireし、enabled 57→54。per-loop telemetryは`4cc7de2` / `23154626`、observe-only CEO unit economicsは`fd7d82d`、標準money event/Gig配線/actual invoice allocatorは`72fc4fd` / `17d1543`、Claude-p bounded recoveryは`70d8269c`、headless/context削減は`dfc9491` / `89764d9` / `5aafa03`、deterministic deliveryは`3311c0c` / `b54e2f9`でPASSする。日次before/afterの確定値はfull JST dayのcoverageを待ち、現時点では固定context A/Bの37.0%減、TIER2診断production A/Bの96.1%減、各production attemptのprovider-reported usageだけを確定値とする。次の即時gateは残るOpenClaw enabled 54件と台帳外launchd 92件を実引数/payloadで照合し、duplicate/deadを1件ずつretire、必要jobだけをshared runnerまたはdeterministic launchdへ移すこと。
