...
TODO #3 launchd control plane: **PASS — declared registry + live read-only inventory**。`profitable-claude` commit `923b12a`は`config/launchd-agents.json`を宣言SSOT、`skills/agent-runner/config.json`を共有model route、`~/Library/LaunchAgents/*.plist`をinstalled cadence、`launchctl`をloaded/disabled live truthとして分離する。read-only CLIは全114 plistを落ちずに走査し、core driver 9件を登録、残り105件を`unregistered`として隠さず表示する。登録済み実測はGig=`hourly at :27`/24回、Claude-p economy=`KeepAlive`、Reddit daily/healthcheck=`disabled`。Apple `plutil`が受理するplistはnative fallbackで読取り、本当に壊れた`ai.anicca.cfo-daily.plist`だけを`invalid-plist`として報告する。モデル/effortは共有runnerから解決し、token/dayはtelemetry前に推定で埋めず`null / unmeasured`とする。inventory 7件とrunner 31件、計38件がgreen。次gateは既存`token-daily-report`、CFO/CEO、runner attempt evidenceを監査し、loop/provider/model/effort/task class単位の実token/cost/revenue ledgerへcopy+tweakすること。Apple `launchd.plist(5)`はLabel/ProgramArguments/KeepAlive/StartInterval/StartCalendarIntervalを定義し、nix-darwin [launchd module](https://github.com/nix-darwin/nix-darwin/blob/master/modules/launchd/launchd.nix)はplistのDisabled値がlive stateを表せないためlaunchdへ問い合わせる必要を明記する。
