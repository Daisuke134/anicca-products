...
- Next minimum action: let the durable IDEA-1→2→3 loop own polling and guarded acquisition. On the first independent purchase, capture delivery/download, finalized Base USDC, provenance-matched ledger row, and positive net margin (B4-V); then repeat with a different buyer/tx and confirm post-settlement Bazaar indexing (B5). Revenue remains `$0 / ¥0` until those events occur.
