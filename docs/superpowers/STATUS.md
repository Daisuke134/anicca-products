...
- Next minimum action: implement IDEA-2 now: consume normalized candidates, verify finalized successful Base USDC Transfer against platform provenance, reject every SELF_WALLET/protocol return/wrong amount, and append the wallet ledger exactly once. Then implement IDEA-3 and let polling own the wait before the first and second independent purchases plus Bazaar indexing.
