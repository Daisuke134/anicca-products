...
- Next minimum action: implement B4-I before waiting: IDEA-1 multi-channel sale observer, IDEA-2 finalized settlement recorder, then IDEA-3 acquisition/repeat controller. After those are durable, let polling own the wait; on first external settlement, match platform provenance to the finalized successful USDC Transfer, reject every SELF_WALLET/protocol return, and record exactly once. Then repeat with a second independent purchase and check Bazaar indexing.
