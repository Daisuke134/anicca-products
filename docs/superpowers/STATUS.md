...
- Next minimum action: implement IDEA-3 now: while sales remain 0, poll eligible the402 demand and conversion signals, execute at most one guarded acquisition action per cycle, and persist the action/outcome so retries cannot spam or duplicate bids. Then let the durable loop own the wait before the first and second independent purchases plus Bazaar indexing.
