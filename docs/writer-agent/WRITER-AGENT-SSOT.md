...
| A7 | ACTIVE | A4-A6 are merged through `be5347f59df2cbf7a893e65481a81ace623dfc7c`, while production still loads the A3 immutable release and `article-daily` remains unloaded. | Build one immutable release from the current merged main SHA, record its tree hash, target only Writer labels against the existing Writer state, and read back installed plist plus loaded launchd arguments. Keep the A3 release and install receipt as rollback evidence. |
