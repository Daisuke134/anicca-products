...
| A3 | COMPLETE | PR `Daisuke134/life-manager#3344` merged as `fc39c2a6a5ad0a6d74207f2717fe0ebacfa4de3a` and first shipped in immutable release `/Users/anicca/loops/releases/20260831T005035-fc39c2a6`. The natural owner selected only ledger-backed run `20260829-165022`, changed `provider-failed-ambiguous -> quality-repair-ready`, and wrote adoption receipt SHA `e4dde41e68ae4a37ac949488e50177df69cf450afa107b2d2fcabf55c5f5c7a6`; competing run `20260830-151951` stayed `interrupted-safe`. A concurrent full-fleet apply then booted the running resume job and produced `entrypoint_exit_143`; PR `#3348` fixed same-effective-plist reapply idempotency and merged as `b73c27ca93b86e629c58fab98d8d78436aeeee63`. Release `/Users/anicca/loops/releases/20260831T011522-b73c27ca` has exact source/release hashes, target install event `de74d004410e4ae2ce8a4f10`, and a second target apply returned `changed=false` without bootout. Natural run `18d0a18e96b358f8-74107` ended pass at event `387f20f78fde2f5586914b5b`; replay logged `action=unchanged` and stopped at the A4 owner boundary. Before/after hashes stayed adoption `e4dde41e...`, generation state `008443b8...`, 325-line ledger `216b8acc...`, and 31-run list `da6e7114...`; attempts stayed three and `article-daily` remained unloaded. | None. Preserve both runs, all attempts, the 325 ledger rows, and the unloaded daily containment until A4 proves the adopted quality path. |
| A4 | COMPLETE | PR `Daisuke134/life-manager#3363` is merged as `fb148993b26a1d5f50e65129c39c7e6c724442cd`. `quality_repair_control.py` now accepts the exact adopted run only after the existing adoption validator returns byte-identical `unchanged/quality-repair-ready`, persists receipt/transition/prompt lineage across `begin`, and classifies only the exact missing-JA-reader traceback as `tracked-reader-terminal-receipt-source-defect`. Normal JSON, a different exception, an existing JA terminal, a missing/hash-drifted receipt, a hand-edited status, and post-begin lineage drift all refuse. The repair prompt still requires editorial, identity, reader, media, CTA, and `quality_self_heal`; `begin` archives the traceback and creates no synthetic quality receipt. Focused evidence is 9 unittest PASS, 3 pytest PASS plus 4 public-effect subtests, shell routing `adopt -> plan -> begin -> model-runner` with no daily start, all eight PR checks green, and final fresh review `ship`. A live `plan` for run `20260829-165022` returned `READY` with receipt/transition/prompt/draft hashes while all 124 run/ledger file hashes remained unchanged. | None. The merged code is intentionally not released or applied until A7; preserve the A3 loaded release, both runs, all attempts, the 325 ledger rows, and unloaded `article-daily`. |
| A5 | COMPLETE | PR `Daisuke134/life-manager#3366` is merged as `fff2351ee800fb5f871e154206caf8c9f91a8ee5`. The start controller now checks only `quality-repair-ready`, reuses the existing adoption validator, and returns `skip-pending-worker / same-jst-day-owned-by-quality-repair` only for exact `unchanged/quality-repair-ready`; missing receipt, prompt/draft drift, public effect, hand-edited or unknown status fall through to `block-incomplete`, while provider-returned behavior is unchanged. The focused portable fixture passes, valid readback leaves generation-state bytes and attempts unchanged, fresh review is `ship`, and all eight PR checks pass. An initial OSS-boundary failure came from changing a baseline-exempt legacy test file containing an existing personal-runtime literal; the test was moved into a new portable file without changing the baseline or weakening the scanner, and the rerun passed. | None. Keep the merged code unreleased until A7 and add only the production-shaped A6 regression next. |
| A6 | COMPLETE | PR `Daisuke134/life-manager#3371` is merged as `be5347f59df2cbf7a893e65481a81ace623dfc7c`. One production-shaped fixture now fixes the exact attempt pattern: attempts 1–2 are `interrupted-safe / 143 / archived-prepublication-artifacts`; attempt 3 is `provider-failed-ambiguous / 1` with generated JA/EN drafts and headline media. It also fixes the four unpublished destination pairs, current JA/EN drafts, headline image, both missing reader terminals, exception-shaped quality file, adoption receipt draft/artifact hashes, named quality owner, and full repair `READY` fields. Adding one real public-effect row makes start control return exact `block-incomplete / same-jst-day-unclassified-run` and repair return `REFUSED / ledger-row-exists`. The 10-test module, OSS boundary, all eight PR checks, and fresh review pass. | None. Preserve the fixture and move to the single merged-main release/install atom. |
...
|---:|---|---|---|
| A8.1 | COMPLETE | `gate-attempt-control.py`, `quality_self_heal.py`, `reader-testing-gate.sh`, `rubric-judge.sh`: bounded current-hash retries, semantic terminal validation, deterministic orphan archive | PR `#3380`; attempt cap stays three; focused shell/Python tests and CI PASS |
| A8.2 | COMPLETE | `article-resume-pending.sh`: when a regular repair state already exists, prevalidate `quality_repair_control plan`, reject wrong run/state/reason, and route prepared/orphaned state to `invoke`, not `begin`; preserve priority publication | PR `#3392`; natural owner passes adoption and reaches `quality-repair-state=invoking` |
| A8.3 | COMPLETE | `editorial-gate.sh`: authorize a changed-hash high evaluation only for the exact active repair run/article and a live ancestor owner; retain per-hash exclusive claim | PR `#3400`; cross-run, symlink, unrelated-PID tests refuse; normal relative CLI remains compatible |
| A8.4 | COMPLETE | `quality_repair_control.py`, resume wrapper: detect the exact terminal-incomplete source defect, create one inode-verified append-only recovery receipt, keep attempts at `2`, refuse a second recovery | PR `#3407`; production wrote `quality-repair-source-recovery.json`; attempts remained `2` |
| A8.5 | COMPLETE | `quality_self_heal.py`: preserve repeated identical orphan snapshots as deterministic `hash`, `hash-2`, `hash-3`; use Darwin `renamex_np(RENAME_EXCL)` / Linux `renameat2(RENAME_NOREPLACE)` | PR `#3410`; 16 focused tests, native no-replace review, all CI PASS |
| A8.6 | **REAPPLY REQUIRED** | `quality_repair_control.py`: add exact owner-prompt source-defect detector; keep attempts `2`; create inode-verified `owner-recovery-1.txt` and `quality-repair-owner-recovery.json`; recover prompt-created/receipt-created/state-bound crashes; bind the reviewed `quality_self_heal.py` SHA. `article-resume-pending.sh`: whitelist only this exact terminal state/reason and pass the distinct invoke-returned prompt to model. Tests: extend `test_prepublication_adoption.py` and `test_article_resume_prepublication_adoption.py`. | First write RED fixtures; then 15 unittest + 17 pytest + persistent shell + syntax/compile/diff PASS; fresh reviewer `ship`; commit/push before cleanup; PR all checks green and admin-merged |
| A8.7 | PENDING | Build APFS clone from a verified full release, overlay the merged Writer paths, verify every tracked blob/mode and all runtime extras, seal read-only; atomically select it and apply with `LIFE_MANAGER_APPLY_TARGET=article-resume` only | Source SHA = release SHA = loaded argv; Connector argv unchanged; install event recorded; stable free bytes ≥ `1,155,780,608` |
| A8.8 | PENDING | Kickstart the existing `ai.anicca.article-resume` owner once; do not run gates or publishers from Codex | Same run `20260829-165022` produces current-hash JA/EN editorial/reader/identity, CTA/media PASS, valid `quality-self-heal.json`, publication-state initialization, external effect still zero before A9 |
| A8.9 | PENDING | Wake the same owner again with unchanged state | Same receipt/effect IDs, no new model recovery receipt, no duplicate publication mutation; then mark A8 COMPLETE and begin A9 |
...
| A8 | Resume the same run through quality | Runtime owner `ai.anicca.article-resume`; `scripts/gate-attempt-control.py`; `scripts/quality_self_heal.py`; `tests/test_quality_iteration_force.py` | Let the existing three-attempt reader budget advance after a current-hash non-pass instead of incorrectly reusing attempt one. When `quality-attempt-N/` exists without `quality-self-heal-attempt-N.json`, preserve it under a deterministic content-hash orphan archive and build a fresh canonical snapshot; never overwrite a committed attempt or raise the cap. Release the fix, then kickstart the existing owner once and watch it. It must create current-hash JA/EN reader/editorial/identity/CTA/media terminals and a valid `quality-self-heal.json`. Codex does not write the article or invoke a substitute publisher. | Existing reader persistence test passes; orphan snapshot regression preserves old bytes and completes attempt one; same run ID reaches valid quality terminal and publication initialization |
