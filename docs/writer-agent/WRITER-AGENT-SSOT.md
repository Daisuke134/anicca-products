...
| A5 | ACTIVE | The exact run is now a receipt-backed quality-repair candidate in merged main, but the daily start controller still lacks an explicit `owned-by-quality-repair` classification for `quality-repair-ready`. | Update only the start-controller classification and its existing focused fixture so the resume worker remains the named owner; unknown states must continue to return `block-incomplete`. |
