...
| A2 | ACTIVE | Existing run `20260829-165022` remains `provider-failed-ambiguous`, has no publication state, and retains its four pending rows. Loading the A1 release naturally started daily run `20260830-151951`; it was stopped before publication and its attempt is durably `interrupted-safe` with rc 143. No state, ledger row, or receipt was deleted. `article-daily` remains unloaded as a safety containment while A2 is active. | Add the hash-bound `adopt-prepublication` transition for the exact existing run, prove all refusal cases and idempotent replay, then restore the scheduled daily label only when the owner routing cannot create a competing run. |
