...
| A3 | ACTIVE | The A2 adoption primitive is present in the release-loaded `article-resume`, but `article-resume-pending.sh` does not yet invoke it before the two quality planners. The existing run is therefore unchanged and no Codex-authored state mutation substitutes for the natural owner. | Wire `adopt-prepublication` before both quality plans, prove ordering/refusal/no-new-run in a focused shell fixture, merge and release it, then let the natural resume owner adopt the exact run and continue in the same wake. |
