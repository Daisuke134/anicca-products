...
| A8 | ACTIVE | The A7 install's natural interval wake ran `ai.anicca.article-resume` once as PID `19843`, owner run `18d0a739e1e41e88-19843`, against only `20260829-165022`. It revised both drafts, produced current-hash JA reader PASS and JA/EN identity PASS, then exited safely with `quality-repair-state.status=retryable-incomplete`, no `quality-self-heal.json`, no publication state, and no external effect. Two exact defects remain: `gate-attempt-control.py` reuses the first current-hash reader `revision-required` terminal instead of consuming the existing three-evaluation budget, so an empty/invalid EN judge response cannot retry; `quality_self_heal.py` reuses attempt number 1 when a crash-left `quality-attempt-1/` snapshot exists without its decision receipt, then rejects the current gate as a conflict. | Keep the existing limits. Let current-hash reader evaluation consume at most the already-defined three attempts, archive any decision-less attempt snapshot under a content-hash-bound orphan path without deletion or overwrite, add one orphan-resume regression, release the fix, then let the natural owner retry the same run. Require current-hash JA/EN reader, editorial, identity, CTA, media, and valid `quality-self-heal.json` before publication initialization. |
