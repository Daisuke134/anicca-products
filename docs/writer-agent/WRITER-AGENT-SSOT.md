...
| A8 | ACTIVE | `ai.anicca.article-resume` is loaded-idle from the A7 release with install event `fa06077e26d96076508cbb66`; Codex has not invoked the model runner or any publisher directly. | Kickstart that existing owner exactly once, watch run `20260829-165022`, and require current-hash JA/EN reader, editorial, identity, CTA, media, and valid `quality-self-heal.json` before publication initialization. |
