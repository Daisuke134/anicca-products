...
| A4 | ACTIVE | Run `20260829-165022` is durably `quality-repair-ready` with a current adoption receipt, exact prompt/draft hashes, three preserved attempts, and no public effect. The resume owner now exits before publication and names quality repair as the next owner. | Update `quality_repair_control.py` so only `provider-returned` or receipt-backed `quality-repair-ready` can return `READY`; refuse a hand-edited status or stale/hash-drifted receipt, and classify the existing missing/exception reader terminal as a tracked repair source without weakening the editorial, identity, reader, media, CTA, or self-heal gates. |
