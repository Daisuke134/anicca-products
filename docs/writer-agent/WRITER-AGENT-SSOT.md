...
| A6 | ACTIVE | A1-A5 behavior is individually covered, but no single fixture yet matches production run `20260829-165022` across its three failed attempts, adopted receipt, exception-shaped quality file, missing reader terminal, current artifacts, and four unpublished ledger rows. | Add exactly the two regression paths in the A6 contract: the full no-effect production shape reaches the named quality owner and repair plan; the same shape with one real public effect refuses. Do not add a broader matrix. |
