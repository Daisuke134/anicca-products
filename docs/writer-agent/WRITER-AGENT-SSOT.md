...
| A8 | ACTIVE | The two quality-retry defects are fixed and merged in PR `#3380` as `3160964a10c05ef00e8fac091e405f2837da32be`. `gate-attempt-control.py`, `quality_self_heal.py`, both gate wrappers, and the two focused test files now preserve a hard three-attempt cap, advance current-hash non-PASS evaluations, archive decision-less snapshots under deterministic content hashes without overwriting evidence, reject contradictory legacy/terminal state, and fail closed when terminal persistence fails. Focused evidence is the persistent shell contract PASS, 24 tests plus 13 subtests PASS, syntax/compile/diff checks PASS, all eight PR checks PASS, and fresh integrity review `ship`. The exact merged tree is installed as immutable release `/Users/anicca/loops/releases/20260831T040131-3160964a`; `ai.anicca.article-resume` plist and loaded argv read back that release and SHA. Three natural owner runs `18d0aaa79fe13db0-98334`, `18d0aaeefac11498-2178`, and `18d0ab35b1294c28-6255` exited before quality mutation with exact blocker `entrypoint_exit_1`: the capacity receipt requires `1,155,780,608` free bytes, while the entrypoint observed `534,044,672`, `491,753,472`, and `320,802,816`. The drop is caused by a concurrent full immutable-release build for merged Connector SHA `bd92ce4f3bfd4e9a76f42531567d80466172008c`, which repeatedly leaves an incomplete non-current release while consuming the Writer reserve. The target run remains `retryable-incomplete`; read-only `quality_repair_control.py plan` returns exact `READY / prepared-quality-repair / attempts=1`. No quality receipt, publication state, provider effect, or external publication was created by these three wakes. | Keep current release `3160964a...`, the measured capacity floor, and all run evidence unchanged. Let the concurrent release owner finish or reclaim its incomplete non-current build through the capacity governor, require stable free space above `1,155,780,608`, then observe the next natural `article-resume` wake. It must consume the prepared repair, create current-hash JA/EN reader/editorial/identity/CTA/media terminals and valid `quality-self-heal.json`, and initialize publication before A9 starts. |
