...
トレンドを元に **投稿 1 本** を書く。X 用 **postText** は **実際に X に載せる 1 ツイート**（**280 文字以内**）。TikTok 用 **caption** は長くてよい（最大 2200 文字）。**reasons** は具体的に（どの tr-xxx、どの pattern を参考にしたか）。**出力する JSON の値はプレースホルダーとする。** listStyle は JSON に含めず、使う場合はプロンプトで指定する。日本市場・日本語前提。
...
（listStyle は JSON に含めない。使う場合はプロンプトで指定する。）
...
**例（JSON の外）:** listStyle 例「完璧主義の苦しみを減らす４つの工夫」「スマホ依存から抜け出す４つの方法」。postText 例は下記。

- postText 例: "30日瞑想してみた。不安に、実際に起きたこと。"（日本市場・日本語）
