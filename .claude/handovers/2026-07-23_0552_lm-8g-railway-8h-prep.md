...
- Railway control plane still blocks a new image/instance. Failed methods and false hypotheses are recorded in §10 row 8g. Latest attempted deployment=`1e5064a8-bcfd-43cf-bac4-c22ff61efe1b` was first observed `INITIALIZING`; fresh CLI `5.28.0` read-only state is now `FAILED`, `deploymentStopped=true`, instances/image/commit absent, `configErrors=["Unknown region: asia-southeast1-eqsg3a"]`. Old healthy deployment=`ea570232-fd20-4614-b2c8-084cb9d3256c`, commit=`d04c522…`, image=`sha256:b7d56d…f9078eb0`, remains SUCCESS with one instance and is untouched.
- Fresh read-only sources: Railway Status (https://status.railway.com/) says `Fully Operational` and `This status page reports incidents with significant, widespread user impact. Smaller or isolated issues won't show up here.` Railway Regions (https://docs.railway.com/deployments/regions; last updated Jul 22, 2026) lists `Southeast Asia Metal | Singapore | asia-southeast1-eqsg3a`; live `regions(projectId:...)` returns exact `name=asia-southeast1-eqsg3a,id=sin,country=Singapore,location=Southeast Asia,deploymentConstraints=null`.
- CLI diagnosis: stale `4.30.5` local `scale --help` panic matches official open issue #874 (https://github.com/railwayapp/cli/issues/874), quote=`Cannot query field "railwayMetal" on type "Region"`. Official `v5.28.0` release (https://github.com/railwayapp/cli/releases/tag/v5.28.0; published `2026-07-22T20:45:41Z`) is installed; `railway --version`=`5.28.0`, and `railway scale --help` exits normally and lists `southeast-asia`/region IDs.
- Fresh live verification: `railway deployment list -s life-call -e production` shows latest `1e5064a8…=FAILED` and active old `ea570232…=SUCCESS`; targeted `railway status --json` gives latest `stopped=true/instances=0/commitHash=null/imageDigest=null`, while active old is `stopped=false/instances=1/d04c522…/sha256:b7d56d…f9078eb0`. Build-log query exits 1 with `Deployment does not have an associated build`; deploy-log query exits 0 with empty output.
- Explicit inference: a broad Railway outage is not evidenced; invalid/deprecated region ID is refuted; stale CLI explains only the local query panic, not the server-side failed deployment. The remaining blocker is a project/deployment-pipeline control-plane inconsistency before build. The status-page caveat leaves an isolated issue possible. Continue read-only monitoring only; do not make a fifth deployment attempt. Recovery gate remains exact-main image + instance + SUCCESS.
