...
| PROP-037b | `deleteWorktree(..., squashPath=true)` is NOT reachable for any record where `unpushedCommits=true` OR `recentActivity=true` (FIND-013: the squash `-D` path is conjoined with the FULL six-predicate non-ACTIVE gate, never a divergent subset): construct a record with `prMergedState='merged'`, `uncommittedChanges=false`, `stashCount=0`, `gitLocked=false`, `detachedHead=false`, `inProgressOp=null` BUT `unpushedCommits=true` → assert `classifyWorktree` returns `active` AND `-D` is never invoked. Repeat with `recentActivity=true`. | REQ-014, REQ-001 | 0 | true | unit test on pure reap-decision fn + call-site assertion |
