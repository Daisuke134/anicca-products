...
test("PANEL-0 Composio disconnect disables only the user account and requires inactive readback", async () => {
  const requests = [], responses = [
    { ok: true, json: async () => ({ items: [{ id: "ca-user-a", status: "ACTIVE", is_disabled: false }] }) },
    { ok: true, json: async () => ({ id: "ca-user-a", is_disabled: true }) },
    { ok: true, json: async () => ({ items: [{ id: "ca-user-a", status: "INACTIVE", is_disabled: true }] }) },
  ];
...
test("PANEL-0 Composio disconnect fails closed on ambiguous ownership or ACTIVE readback", async () => {
  ...
  await assert.rejects(composioCalendarDisconnect({ uid: "u-a", chatId: "101" }, { composioKey: "test-key", fetchImpl: async () => response([{ id: "one", status: "ACTIVE" }, { id: "two", status: "ACTIVE" }]) }), /provider_ambiguous/);
  const sequence = [response([{ id: "one", status: "ACTIVE" }]), { ok: true, json: async () => ({}) }, response([{ id: "one", status: "ACTIVE" }])];
...
test("PANEL-0 Composio reconnect enables the scoped inactive account and requires ACTIVE readback", async () => {
  ...
  const sequence = [response([{ id: "ca-user-a", status: "INACTIVE", is_disabled: true }]), { ok: true, json: async () => ({}) }, response([{ id: "ca-user-a", status: "ACTIVE", is_disabled: false }])];
