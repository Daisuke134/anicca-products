...
const OFFERS = { elevenlabs: { env: 'AFFILIATE_ELEVENLABS_URL', host: 'try.elevenlabs.io' } };
...
function validSignature(secret, fields, supplied) {
  ...
  return crypto.timingSafeEqual(
    Buffer.from(signAttribution(secret, fields), 'hex'),
...
function targetFor(offer) {
  ...
  try {
    ...
    return target.protocol === 'https:' && target.hostname === config.host ? target.toString() : null;
  } catch {
    return null;
  }
}
...
router.get('/go/:offer', async (req, res) => {
  const fields = {
    offer: clean(req.params.offer), placement: clean(req.query.placement),
    locale: clean(req.query.locale), experiment: clean(req.query.experiment),
    variant: clean(req.query.variant),
  };
  const target = targetFor(fields.offer);
  if (!target || Object.values(fields).some(value => !value) ||
  ...
  try {
    await prisma.opsEvent.create({
      data: {
        source: 'affiliate-agent', kind: 'affiliate_click',
