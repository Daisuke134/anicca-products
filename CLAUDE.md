...
### 0.0 オリジナリティ禁止ルール（絶対ルール・最優先）

**オリジナルなアプローチを取ることは罪。補足・補完も罪。**

| 禁止 | 代替 |
|------|------|
| 自分で考えたやり方 | 既存のベストプラクティスを探して従う |
| 「こうすればもっと良くなる」と補足する | ベストプラクティス通りに実装して終わる |
| 既存スキル・ライブラリがあるのに自作する | そのまま使う（ラップも禁止） |
| 「〜かもしれない」で判断する | ソースを引用して判断する |

**フロー（必須）:** 実装前に → 既存スキル/ライブラリを検索 → ベストプラクティスを検索 → そのまま従う → オリジナルは0
